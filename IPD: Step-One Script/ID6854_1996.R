#load dataset
library('foreign')
demographics<-read.spss('H96A_R.SAV')
depression<-read.spss('H96B_R.SAV')
income<-read.spss('H96J_H.SAV')
employment<-read.spss('H96G_R.SAV')
other<-read.spss('H96CS_R.SAV')
#Merge datasets by Case ID 
ID68541996<-merge(demographics,income,by="HHID")
ID68541996$HHIDPN<-paste(ID68541996$HHID,ID68541996$PN,sep = "")
depression$HHIDPN<-paste(depression$HHID,depression$PN,sep = "")
ID68541996<-merge(ID68541996,depression,by="HHIDPN")
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68541996<-merge(ID68541996,employment,by="HHIDPN")
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68541996<-merge(ID68541996,other,by="HHIDPN")
#Coded variables
...
#Birth Month: E636
#Birth Day: NA
#Evaluation Year: E397
#Evaluation Month: E398
#Evaluation Day: NA
#Participant Income: NA  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: E4643 
#Participant Education: E649
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: E2732M 
...
ID6854DS0<-data.frame(HHIDPN,E985,E986,E987,E988,E989,E990,E991,E992,E664,E667M,
                      E374,E638,E636,E397,E398,E4643,E649,E2732M)
detach(ID68541996)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68541996,demographics,depression,employment,income,other)
...
#1=YES
#5=NO
ID6854DS0$LAT<-ifelse(ID6854DS0$E664==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#7=OTHER (SPECIFY);
#8=DK (don't know);
#9=RF (refused)
ID6854DS0$BLK<-ifelse(ID6854DS0$E667M==2,1,0)
ID6854DS0$OTH<-ifelse(ID6854DS0$E667M==7,1,0)
ID6854DS0$WHT<-ifelse(ID6854DS0$E667M==1,1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$E374==1,1,
               ifelse(ID6854DS0$E374==2,0,NA))

#Recode age 
ID6854DS0$E638[ID6854DS0$E638=="9999"]<-NA
ID6854DS0$E638[ID6854DS0$E638=="9998"]<-NA
ID6854DS0$E397[ID6854DS0$E397=="1980"]<-NA #not a feasible date
ID6854DS0$AGECALC<-ID6854DS0$E397-ID6854DS0$E638

#Income, Education, Occupation ----
##Income
ID6854DS0$E4643[ID6854DS0$E4643=="9999997"]<-NA
ID6854DS0$E4643[ID6854DS0$E4643=="9999998"]<-NA
ID6854DS0$E4643[ID6854DS0$E4643=="9999999"]<-NA
ID6854DS0$INC<-ID6854DS0$E4643
        
##Education
ID6854DS0$E649[ID6854DS0$E649=="97"]<-NA
ID6854DS0$E649[ID6854DS0$E649=="98"]<-NA
ID6854DS0$E649[ID6854DS0$E649=="99"]<-NA
ID6854DS0$EDU<-ID6854DS0$E649
...
write.csv(ID6854DS0, file = 'ID6854DS0_1996_recoded.csv')
...
ID6854DS0_1996_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_1996_aggregated, file = 'ID6854DS0_1996_aggregated.csv',row.names = FALSE)
