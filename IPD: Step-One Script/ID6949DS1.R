#load dataset
library('foreign')
data<-read.spss('06949-0001-Data.sav')
#Coded variables
#Participant ID: NA
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#FLTEAT,FLTMIND,FLTDEPRS,FLTEFFRT,FLTSLEEP,FLTSAD,FLTGOING
...
#Age: IRAGE
#Birth Year: BIRYEAR	
#Birth Month: BIRMONTH
#Birth Day: NA
#Evaluation Year: NA
#Evaluation Month: NA
#Evaluation Day: NA
#Participant Income: INDINC  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: TOTINC 
#Participant Education: IREDUC
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: ROCCUP2 
...
ID6949DS1<-data.frame(FLTEAT,FLTMIND,FLTDEPRS,FLTEFFRT,FLTSLEEP,FLTSAD,
                      FLTGOING,IRHOIND,IRHOGRP2,IRRACE,IRSEX,IRAGE,BIRYEAR,
                      BIRMONTH,INDINC,TOTINC,IREDUC,ROCCUP2)
detach(data)
ID6949DS1$ORIGOBS<-nrow(ID6949DS1)
rm(data)
...
ID6949DS1$FLTEAT<-ifelse(ID6949DS1$FLTEAT == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTEAT == "Some or a little of the time (1-2 days)", 1, 
                  ifelse(ID6949DS1$FLTEAT == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTEAT == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTMIND<-ifelse(ID6949DS1$FLTMIND == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTMIND == "Some or a little of the time (1-2 days)", 1, 
                   ifelse(ID6949DS1$FLTMIND == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTMIND == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTDEPRS<-ifelse(ID6949DS1$FLTDEPRS == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTDEPRS == "Some or a little of the time (1-2 days)", 1, 
                    ifelse(ID6949DS1$FLTDEPRS == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTDEPRS == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTEFFRT<-ifelse(ID6949DS1$FLTEFFRT == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTEFFRT == "Some or a little of the time (1-2 days)", 1, 
                    ifelse(ID6949DS1$FLTEFFRT == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTEFFRT == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTSLEEP<-ifelse(ID6949DS1$FLTSLEEP == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTSLEEP == "Some or a little of the time (1-2 days)", 1, 
                    ifelse(ID6949DS1$FLTSLEEP == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTSLEEP == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTSAD<-ifelse(ID6949DS1$FLTSAD == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTSAD == "Some or a little of the time (1-2 days)", 1, 
                  ifelse(ID6949DS1$FLTSAD == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTSAD == "Most or all of the time (5-7 days)", 3, NA))))
ID6949DS1$FLTGOING<-ifelse(ID6949DS1$FLTGOING == "Rarely or none of the time (less than a day)", 0, ifelse(ID6949DS1$FLTGOING == "Some or a little of the time (1-2 days)", 1, 
                    ifelse(ID6949DS1$FLTGOING == "Occasion. or a moderate amount of time (3-4 days)", 2, ifelse(ID6949DS1$FLTGOING == "Most or all of the time (5-7 days)", 3, NA))))
...
dsitems <- ID6949DS1[c(1:7)]
#dsitemspw <- ID6949DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID6949DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID6949DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID6949DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID6949DS1$LAT<-ifelse(ID6949DS1$IRHOIND == "Hispanic", 1, 0)
ID6949DS1$WHT<-ifelse(ID6949DS1$IRRACE == "White", 1, 0)
ID6949DS1$BLK<-ifelse(ID6949DS1$IRRACE == "Black", 1, 0)
ID6949DS1$AI<-ifelse(ID6949DS1$IRRACE == "American indian or Alaskan native", 1, 0)
ID6949DS1$AA<-ifelse(ID6949DS1$IRRACE == "Asian or Pacific islander", 1, 0)
ID6949DS1$OTH<-NA

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
ID6949DS1$MR<-ifelse((rowSums(x=cbind(ID6949DS1$WHT,ID6949DS1$BLK,ID6949DS1$AI,ID6949DS1$AA,ID6949DS1$OTH),
                              na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
ID6949DS1$WHT<-ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 0)&(ID6949DS1$WHT == 1)), 1, 0)
ID6949DS1$BLK<-ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 0)&(ID6949DS1$BLK == 1)), 1, 0)
ID6949DS1$AI<-ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 0)&(ID6949DS1$AI == 1)), 1, 0)
ID6949DS1$AA<-ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 0)&(ID6949DS1$AA == 1)), 1, 0)
ID6949DS1$OTH<-NA#ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 0)&(ID6949DS1$OTH == 1)), 1, 0)
ID6949DS1$MR<-ifelse(((ID6949DS1$LAT == 0)&(ID6949DS1$MR == 1)), 1, 0)

#Recode factor into numeric,
ID6949DS1$SEX<-ifelse(ID6949DS1$IRSEX == "Male", 1, 0)

#Recode age 
ID6949DS1$AGECALC<-ID6949DS1$IRAGE

#Income, Education, Occupation ----
##Income
levels(ID6949DS1$TOTINC)[levels(ID6949DS1$TOTINC)=="No family income"]<-0
ID6949DS1$TOTINC<-as.numeric(as.character(ID6949DS1$TOTINC))
#take highest income reported
ID6949DS1$INC<-pmax(ID6949DS1$INDINC,ID6949DS1$TOTINC,na.rm = TRUE)
        
##Education
ID6949DS1$EDU<-ifelse(ID6949DS1$IREDUC=="No schooling",0,                
               ifelse(ID6949DS1$IREDUC=="First grade",1,                 
               ifelse(ID6949DS1$IREDUC=="Second grade",2,                 
               ifelse(ID6949DS1$IREDUC=="Third grade",3,                 
               ifelse(ID6949DS1$IREDUC=="Fourth grade",4,                 
               ifelse(ID6949DS1$IREDUC=="Fifth grade",5,                 
               ifelse(ID6949DS1$IREDUC=="Sixth grade",6,                  
               ifelse(ID6949DS1$IREDUC=="Seventh grade",7,               
               ifelse(ID6949DS1$IREDUC=="Eighth grade",8,                 
               ifelse(ID6949DS1$IREDUC=="Ninth grade",9,                 
               ifelse(ID6949DS1$IREDUC=="Tenth grade",10,                  
               ifelse(ID6949DS1$IREDUC=="Eleventh grade",11,              
               ifelse(ID6949DS1$IREDUC=="Twelfth grade",12,                
               ifelse(ID6949DS1$IREDUC=="One year of college",13,         
               ifelse(ID6949DS1$IREDUC=="Two years of college",14,         
               ifelse(ID6949DS1$IREDUC=="Three years of college",15,      
               ifelse(ID6949DS1$IREDUC=="Four years of college",16,        
               ifelse(ID6949DS1$IREDUC=="Graduate/professional school",17,NA))))))))))))))))))
...
write.csv(ID6949DS1, file = 'ID6949DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symtpoms items
ID6949DS1 <- ID6949DS1[which(ID6949DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID6949DS1$AGECALC)
dstatsdep<-describe(ID6949DS1$DEP)
dstatsachv<-NA#describe(ID6949DS1$ACHV)
dstatsinc<-describe(ID6949DS1$INC)
dstatsedu<-describe(ID6949DS1$EDU)
dstatsoccs<-NA#describe(ID6949DS1$OCCS)
dstatsoccp<-NA#describe(ID6949DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID6949DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID6949DS1_dephist.pdf')
hist(ID6949DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID6949DS1_achvhist.pdf')
#hist(ID6949DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID6949DS1_inchist.pdf')
hist(ID6949DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID6949DS1_educhist.pdf')
hist(ID6949DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID6949DS1_occshist.pdf')
#hist(ID6949DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID6949DS1_occphist.pdf')
#hist(ID6949DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID6949DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID6949DS1[which((ID6949DS1$AGECALC >= 5)&(ID6949DS1$AGECALC <= 11)), ] 
AD<-ID6949DS1[which((ID6949DS1$AGECALC >= 12)&(ID6949DS1$AGECALC <= 17)), ]
YD<-ID6949DS1[which((ID6949DS1$AGECALC >= 18)&(ID6949DS1$AGECALC <= 25)), ]
AT<-ID6949DS1[which((ID6949DS1$AGECALC >= 26)&(ID6949DS1$AGECALC <= 64)), ]
OA<-ID6949DS1[which(ID6949DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID6949DS1, then DATAID = 21600)
#DATAID<-'6949'
#names(DATAID)<-'DATAID'
...
#ALPHAOV <- ci.reliability(data=WHTCHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=WHTCHM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTADF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTADF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTADM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTADM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
...
#names(DATAYEAR)<-'DATAYEAR'
##Compute reliability estimates
#ALPHAOV <- ci.reliability(data=LATCHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATCHM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
names(DATAYEAR)<-'DATAYEAR'
#Compute reliability estimates
ALPHAOV <- ci.reliability(data=LATADF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATADF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATADM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATADM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATYDF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATYDF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATYDM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATYDM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATOAF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATOAF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=LATOAM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=LATOAM[1:7], type = "omega", interval.type = "none")
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKADF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKADF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKADM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKADM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKYDF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKYDF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKYDM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKYDM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATM[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKOAF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKOAF[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=BLKOAM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=BLKOAM[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHMC[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHMC[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAADF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAADF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAADM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAADM[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAYDF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAYDF[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAYDM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAYDM[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAATF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAATF[1:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAATM[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAATM[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AIATF[1:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AIATF[1:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIATM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIATM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAF[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAF[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAM[1:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAM[1:7], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID6949DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID6949DS1_aggregated, file = 'ID6949DS1_aggregated.csv',row.names = FALSE)
