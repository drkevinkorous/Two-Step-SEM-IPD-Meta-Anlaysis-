#load dataset
library('foreign')
#H08B_R.SAV (demographics) is partial data (a lot missing), merge with prior wave 
demographics<-read.spss('H08B_R.SAV')
demographicspw<-as.data.frame(read.spss('H06B_R.SAV'))
demographicspw<-demographicspw[,c(1,2,42,46,47,48)]
demographics$HHIDPN<-paste(demographics$HHID,demographics$PN,sep = "")
demographicspw$HHIDPN<-paste(demographicspw$HHID,demographicspw$PN,sep = "")
demographics<-merge(demographics,demographicspw,by="HHIDPN",all.x = TRUE)
depression<-read.spss('H08D_R.SAV')
income<-read.spss('H08Q_H.sav')
employment<-read.spss('H08J_R.SAV')
other<-read.spss('H08A_R.SAV')
preload<-read.spss('H08PR_R.SAV')
#merge with recoded data from 2006
education<-read.csv(file = 'ID6854DS0_2006_recoded.csv') #(from 2004, most complete data)
education<-education[,c(2,48)]
#Merge datasets
ID68542008<-merge(depression,income,by="HHID",all=TRUE)
ID68542008$HHIDPN<-paste(ID68542008$HHID,ID68542008$PN,sep = "")
ID68542008<-merge(ID68542008,demographics,by="HHIDPN",all=TRUE)
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68542008<-merge(ID68542008,employment,by="HHIDPN",all = TRUE)
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68542008<-merge(ID68542008,other,by="HHIDPN",all = TRUE)
preload$HHIDPN<-paste(preload$HHID,preload$PN,sep = "")
ID68542008<-merge(ID68542008,preload,by="HHIDPN",all=TRUE)
ID68542008<-merge(ID68542008,education,by="HHIDPN",all=TRUE)
#Merge prior wave dep symptoms
depressionpw<-read.spss('H06D_R.SAV')
depressionpw$HHIDPN<-paste(depressionpw$HHID,depressionpw$PN,sep = "")
ID68542008<-merge(ID68542008,depressionpw,by="HHIDPN",all.x = TRUE)
#Coded variables
...
#Ethnicity: LB028 prior wave: KB028 
#Race: LB089M1M,LB089M2M,LB089M3M  prior wave: KB089M1M,KB089M2M,KB089M3M  
#White: NA
#Black: NA 
...
#Birth Month: LX004_R
#Birth Day: NA
#Evaluation Year: LA501
#Evaluation Month: LA500
#Evaluation Day: NA
#Participant Income: LQ020  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: LQ020+LQ045(partner)
#Participant Education: LB014A prior wave: EDU
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: LJ062M 
...
ID6854DS0<-data.frame(HHIDPN,LD110,LD111,LD112,LD113,LD114,LD115,LD116,LD117,
                      KD110,KD111,KD112,KD113,KD114,KD115,KD116,KD117,KB028,LB028,
                      LB089M1M,LB089M2M,LB089M3M,KB089M1M,KB089M2M,KB089M3M,LX060_R,
                      LA019,LX067_R,LX004_R,LA501,LA500,LQ020,LQ045,LB014,EDU,LJ062M)
detach(ID68542008)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68542008,demographics,depression,employment,income,other,preload,depressionpw,education,demographicspw)
...
#9=RF (refused)
ID6854DS0$LD110<-ifelse(ID6854DS0$LD110==1,1,
                ifelse(ID6854DS0$LD110==5,0,NA))
ID6854DS0$LD111<-ifelse(ID6854DS0$LD111==1,1,
                ifelse(ID6854DS0$LD111==5,0,NA))                 
ID6854DS0$LD112<-ifelse(ID6854DS0$LD112==1,1,
                ifelse(ID6854DS0$LD112==5,0,NA))
ID6854DS0$LD113<-ifelse(ID6854DS0$LD113==1,0,
                ifelse(ID6854DS0$LD113==5,1,NA))#RC
ID6854DS0$LD114<-ifelse(ID6854DS0$LD114==1,1,
                ifelse(ID6854DS0$LD114==5,0,NA))
ID6854DS0$LD115<-ifelse(ID6854DS0$LD115==1,0,
                ifelse(ID6854DS0$LD115==5,1,NA))#RC
ID6854DS0$LD116<-ifelse(ID6854DS0$LD116==1,1,
                ifelse(ID6854DS0$LD116==5,0,NA))
ID6854DS0$LD117<-ifelse(ID6854DS0$LD117==1,1,
                 ifelse(ID6854DS0$LD117==5,0,NA))
...
#1=YES
#5=NO
ID6854DS0$LB028<-ifelse(is.na(ID6854DS0$KB028),ID6854DS0$LB028,ID6854DS0$KB028)
ID6854DS0$LAT<-ifelse(ID6854DS0$LB028==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#97=OTHER (SPECIFY);
#98=DK (don't know);
#99=RF (refused)
ID6854DS0$LB089M1M<-ifelse(is.na(ID6854DS0$KB089M1M),ID6854DS0$LB089M1M,ID6854DS0$KB089M1M)
ID6854DS0$LB089M2M<-ifelse(is.na(ID6854DS0$KB089M2M),ID6854DS0$LB089M2M,ID6854DS0$KB089M2M)
ID6854DS0$LB089M3M<-ifelse(is.na(ID6854DS0$KB089M3M),ID6854DS0$LB089M3M,ID6854DS0$KB089M3M)
ID6854DS0$BLK<-ifelse(((ID6854DS0$LB089M1M==2)|(ID6854DS0$LB089M2M==2)|(ID6854DS0$LB089M3M==2)),1,0)
ID6854DS0$OTH<-ifelse(((ID6854DS0$LB089M1M==97)|(ID6854DS0$LB089M2M==97)|(ID6854DS0$LB089M3M==97)),1,0)
ID6854DS0$WHT<-ifelse(((ID6854DS0$LB089M1M==1)|(ID6854DS0$LB089M2M==1)|(ID6854DS0$LB089M3M==1)),1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$LX060_R==1,1,
               ifelse(ID6854DS0$LX060_R==2,0,NA))

#Recode age 
ID6854DS0$LA019[ID6854DS0$LA019=="0"]<-NA #age of zero is out of bounds
ID6854DS0$AGECALC<-ID6854DS0$LA019

#Income, Education, Occupation ----
##Income
ID6854DS0$LQ020[ID6854DS0$LQ020=="99999998"]<-NA
ID6854DS0$LQ020[ID6854DS0$LQ020=="99999999"]<-NA
ID6854DS0$LQ045[ID6854DS0$LQ045=="99999998"]<-NA
ID6854DS0$LQ045[ID6854DS0$LQ045=="99999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$LQ020,ID6854DS0$LQ045),na.rm = TRUE)
        
##Education
ID6854DS0$LB014[ID6854DS0$LB014=="97"]<-NA
#incorporate prior wave
ID6854DS0$EDU<-pmax(ID6854DS0$EDU,ID6854DS0$LB014,na.rm=TRUE)
...
write.csv(ID6854DS0, file = 'ID6854DS0_2008_recoded.csv')
...
ID6854DS0_2008_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_2008_aggregated, file = 'ID6854DS0_2008_aggregated.csv',row.names = FALSE)
