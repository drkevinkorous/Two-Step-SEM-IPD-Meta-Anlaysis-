...
#V10283,V10284,V10285,V10286(RC),V10287,V10288,V10289(RC),V10290,V10291,V10292,V10293
...
#Birth Month: NA
#Birth Day: NA
#Evaluation Year: V12005
#Evaluation Month: V12004
#Evaluation Day: V12003
#Participant Income: NA  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: V13113
...
#Participant Occupation: V13311, V11318 & V7018 & V1112 (prior waves)
...
ID4690DS1<-data.frame(V12325,V12326,V12327,V12328,V12329,V12330,V12331,V12332,V12333,V12334,V12335,
                      V10283,V10284,V10285,V10286,V10287,V10288,V10289,V10290,V10291,V10292,V10293,
                      V1620,V2004,V1628,V1629,V1631,V1630,V1632,V1801,V2001,V12005,V12004,V12003,
                      V13113,V2007,V13311,V11318,V7018,V1112) 
detach(da04690.0001)
...
ID4690DS1$V10283<-ifelse(ID4690DS1$V10283=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10283=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10283=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10284<-ifelse(ID4690DS1$V10284=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10284=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10284=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10285<-ifelse(ID4690DS1$V10285=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10285=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10285=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10286<-ifelse(ID4690DS1$V10286=="(1) HARDEVER",2,
                  ifelse(ID4690DS1$V10286=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10286=="(3) MOSTTIME",0,NA))) #RC
ID4690DS1$V10287<-ifelse(ID4690DS1$V10287=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10287=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10287=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10288<-ifelse(ID4690DS1$V10288=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10288=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10288=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10289<-ifelse(ID4690DS1$V10289=="(1) HARDEVER",2,
                  ifelse(ID4690DS1$V10289=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10289=="(3) MOSTTIME",0,NA))) #RC
ID4690DS1$V10290<-ifelse(ID4690DS1$V10290=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10290=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10290=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10291<-ifelse(ID4690DS1$V10291=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10291=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10291=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10292<-ifelse(ID4690DS1$V10292=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10292=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10292=="(3) MOSTTIME",2,NA)))
ID4690DS1$V10293<-ifelse(ID4690DS1$V10293=="(1) HARDEVER",0,
                  ifelse(ID4690DS1$V10293=="(2) SOMETIME",1,
                  ifelse(ID4690DS1$V10293=="(3) MOSTTIME",2,NA)))
...
#Recode age 
ID4690DS1$AGECALC<-ID4690DS1$V12005-ID4690DS1$V2001

#Income, Education, Occupation ----
##Income
ID4690DS1$INC<-ID4690DS1$V13113 

##Education
ID4690DS1$EDU<-ID4690DS1$V2007

##Occupation
#merge with prior waves (respondents only answered if job changed) 
ID4690DS1$occpw<-ifelse((is.na(ID4690DS1$V7018)),ID4690DS1$V1112,ID4690DS1$V7018)
ID4690DS1$occpw<-ifelse((is.na(ID4690DS1$V11318)),ID4690DS1$occpw,ID4690DS1$V11318)
ID4690DS1$occ<-ifelse((is.na(ID4690DS1$V13311)),ID4690DS1$occpw,ID4690DS1$V13311)
#Nam-Powers-Boyd scale (Nam & Boyd, 2008)
occ_codes<-read.csv(file = 'ID4690DS1_2002_Occupational codes.csv')
#match codes with each row in data
ID4690DS1$OCCS<-occ_codes$OCC_STATUS[ match(ID4690DS1$occ,occ_codes$OCC_CODE)]

#Recode factor into numeric (occupational prestige)
#Nakao & Tres scale (1994)
ID4690DS1$OCCP<-occ_codes$OCC_PREST[ match(ID4690DS1$occ,occ_codes$OCC_CODE)]

#Save Recoded Dataset ------
#Save database as .csv for OSF 
write.csv(ID4690DS1, file = 'ID4690DS1_2002_recoded.csv')
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATOAM[1:11], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATOAM[1:11], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID4690DS1_2002_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID4690DS1_2002_aggregated, file = 'ID4690DS1_2002_aggregated.csv',row.names = FALSE)
