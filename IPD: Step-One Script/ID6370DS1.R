#Load data
library('foreign')
da06370.0001<-read.spss('06370-0001-Data.sav')
#Coded variables
#Participant ID: NA
#Measure Name (not a variable in the dataset): SF36
#Depressive symptom items: 
#EFT01(RC),EFT02(RC),EFT03,EFT04,MHA01,MHC01,MHP03(RC),MHD02,MHP01(RC)
...
#Birth Month: DOBMO
#Birth Day: DOBDY
#Evaluation Year: INTYR
#Evaluation Month: INTMO
#Evaluation Day: INTDY
#Participant Income: NA  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: PPINCIMP,Q43
#Participant Education: EDUC
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: OCCUPC 
...
#Notes: National Survey of Functional Health Status, 1990

#Create Dataset ----
attach(da06370.0001)
#Extract relevant variables into a dataframe.
ID6370DS1<-data.frame(EFT01,EFT02,EFT03,EFT04,MHA01,MHC01,MHP03,MHD02,MHP01,RACE,SEX,
                      AGE,DOBYR,DOBMO,DOBDY,INTYR,INTMO,INTDY,EDUC,OCCUPC)
detach(da06370.0001)
ID6370DS1$ORIGOBS<-nrow(ID6370DS1)
rm(da06370.0001)
...
ID6370DS1$EFT01<-dplyr::recode(ID6370DS1$EFT01,'1'=0,'2'=1,'3'=2,'4'=3,'5'=4,'6'=5)#RC
ID6370DS1$EFT02<-dplyr::recode(ID6370DS1$EFT02,'1'=0,'2'=1,'3'=2,'4'=3,'5'=4,'6'=5)#RC
ID6370DS1$EFT03<-dplyr::recode(ID6370DS1$EFT03,'1'=5,'2'=4,'3'=3,'4'=2,'5'=1,'6'=0)
ID6370DS1$EFT04<-dplyr::recode(ID6370DS1$EFT04,'1'=5,'2'=4,'3'=3,'4'=2,'5'=1,'6'=0)
ID6370DS1$MHA01<-dplyr::recode(ID6370DS1$MHA01,'1'=5,'2'=4,'3'=3,'4'=2,'5'=1,'6'=0)
ID6370DS1$MHC01<-dplyr::recode(ID6370DS1$MHC01,'1'=5,'2'=4,'3'=3,'4'=2,'5'=1,'6'=0)
ID6370DS1$MHP03<-dplyr::recode(ID6370DS1$MHP03,'1'=0,'2'=1,'3'=2,'4'=3,'5'=4,'6'=5)#RC
ID6370DS1$MHD02<-dplyr::recode(ID6370DS1$MHD02,'1'=5,'2'=4,'3'=3,'4'=2,'5'=1,'6'=0)
ID6370DS1$MHP01<-dplyr::recode(ID6370DS1$MHP01,'1'=0,'2'=1,'3'=2,'4'=3,'5'=4,'6'=5)#RC
...
dsitems <- ID6370DS1[c(1:9)]
#dsitemspw <- ID6370DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID6370DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID6370DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID6370DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID6370DS1$LAT<-ifelse(ID6370DS1$RACE==3,1,0)
ID6370DS1$WHT<-ifelse(ID6370DS1$RACE==1,1,0)
ID6370DS1$BLK<-ifelse(ID6370DS1$RACE==2,1,0)
ID6370DS1$AI<-NA
ID6370DS1$AA<-ifelse(ID6370DS1$RACE==4,1,0)
ID6370DS1$OTH<-ifelse(ID6370DS1$RACE==5,1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID6370DS1$MR<-NA#ifelse((rowSums(x=cbind(ID6370DS1$WHT,ID6370DS1$BLK,ID6370DS1$AI,ID6370DS1$AA,
#                                       ID6370DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
#ID6370DS1$WHT<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 0)&(ID6370DS1$WHT == 1)), 1, 0)
#ID6370DS1$BLK<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 0)&(ID6370DS1$BLK == 1)), 1, 0)
#ID6370DS1$AI<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 0)&(ID6370DS1$AI == 1)), 1, 0)
#ID6370DS1$AA<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 0)&(ID6370DS1$AA == 1)), 1, 0)
#ID6370DS1$OTH<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 0)&(ID6370DS1$OTH == 1)), 1, 0)
#ID6370DS1$MR<-ifelse(((ID6370DS1$LAT == 0)&(ID6370DS1$MR == 1)), 1, 0)

#Recode sex
#0=female, 1=male
ID6370DS1$SEX<-ID6370DS1$SEX
                
#Recode age 
ID6370DS1$AGECALC<-ID6370DS1$AGE
...
write.csv(ID6370DS1, file = 'ID6370DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID6370DS1 <- ID6370DS1[which(ID6370DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID6370DS1$AGECALC)
dstatsdep<-describe(ID6370DS1$DEP)
dstatsachv<-NA#describe(ID6370DS1$ACHV)
dstatsinc<-NA#describe(ID6370DS1$INC)
dstatsedu<-describe(ID6370DS1$EDU)
dstatsoccs<-NA#describe(ID6370DS1$OCCS)
dstatsoccp<-NA#describe(ID6370DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID6370DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID6370DS1_dephist.pdf')
hist(ID6370DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID6370DS1_achvhist.pdf')
#hist(ID6370DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
#pdf('ID6370DS1_inchist.pdf')
#hist(ID6370DS1$INC, xlab="Income", main = NULL, col="grey")
#dev.off()
pdf('ID6370DS1_educhist.pdf')
hist(ID6370DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID6370DS1_occshist.pdf')
#hist(ID6370DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID6370DS1_occphist.pdf')
#hist(ID6370DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID6370DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID6370DS1[which((ID6370DS1$AGECALC >= 5)&(ID6370DS1$AGECALC <= 11)), ] 
#AD<-ID6370DS1[which((ID6370DS1$AGECALC >= 12)&(ID6370DS1$AGECALC <= 17)), ]
YD<-ID6370DS1[which((ID6370DS1$AGECALC >= 18)&(ID6370DS1$AGECALC <= 25)), ]
AT<-ID6370DS1[which((ID6370DS1$AGECALC >= 26)&(ID6370DS1$AGECALC <= 64)), ]
OA<-ID6370DS1[which(ID6370DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID6370DS1, then DATAID = 21600)
#DATAID<-'6370'
#names(DATAID)<-'DATAID'
...
#ALPHAOV <- ci.reliability(data=WHTCHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=WHTCHM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=WHTADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=WHTADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#names(DATAYEAR)<-'DATAYEAR'
##Compute reliability estimates
#ALPHAOV <- ci.reliability(data=LATCHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATCHM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#names(DATAYEAR)<-'DATAYEAR'
##Compute reliability estimates
#ALPHAOV <- ci.reliability(data=LATADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATYDF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATYDF[1:9], type = "omega", interval.type = "none")
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATYDM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATYDM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATOAF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATOAF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATOAM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATOAM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKYDF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKYDF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=BLKYDM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=BLKYDM[1:9], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKOAF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKOAF[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKOAM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKOAM[1:9], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHMC[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHMC[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAYDF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAYDF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAYDM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAYDM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAATF[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAATF[1:9], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAATM[1:9], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAATM[1:9], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIATF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIATF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIATM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIATM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAF[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAF[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAM[1:9], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAM[1:9], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID6370DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6370DS1_aggregated, file = 'ID6370DS1_aggregated.csv',row.names = FALSE)