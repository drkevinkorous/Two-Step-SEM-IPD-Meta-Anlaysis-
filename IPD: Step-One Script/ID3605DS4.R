#load dataset
library('foreign')
da03605.0004<-read.spss('03605-0004-Data.sav') #Data was downloaded in ASCII format and setup in SPSS
#merge with income data
da03605.0002<-read.spss('03605-0002-Data.sav') #Data was downloaded in ASCII format and setup in SPSS
merged3605<-merge(da03605.0004,da03605.0002,by="hhx")
#Coded variables
#Participant ID: hhx
#Measure Name (not a variable in the dataset): K6
#Depressive symptom items: 
#Ssad,nervous,restless,hopeless,effort,worthls
...
ID3605DS4<-data.frame(hhx,sad,nervous,restless,hopeless,effort,worthls,hispan_i,rcdt1p_i,
                      sex,age_p,srvy_yr.x,fincgrp,educ,occup1) 
detach(merged3605)
colnames(ID3605DS4)[colnames(ID3605DS4)=="srvy_yr.x"]<-"srvy_yr"
ID3605DS4$ORIGOBS<-nrow(ID3605DS4)
rm(da03605.0004,da03605.0002)
...
ID3605DS4$sad<-ifelse(ID3605DS4$sad=="ALL of the time",4,
                  ifelse(ID3605DS4$sad=="MOST of the time",3,
                  ifelse(ID3605DS4$sad=="SOME of the time",2,
                  ifelse(ID3605DS4$sad=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$sad=="NONE of the time",0,NA)))))
ID3605DS4$nervous<-ifelse(ID3605DS4$nervous=="ALL of the time",4,
                  ifelse(ID3605DS4$nervous=="MOST of the time",3,
                  ifelse(ID3605DS4$nervous=="SOME of the time",2,
                  ifelse(ID3605DS4$nervous=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$nervous=="NONE of the time",0,NA)))))
ID3605DS4$restless<-ifelse(ID3605DS4$restless=="ALL of the time",4,
                  ifelse(ID3605DS4$restless=="MOST of the time",3,
                  ifelse(ID3605DS4$restless=="SOME of the time",2,
                  ifelse(ID3605DS4$restless=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$restless=="NONE of the time",0,NA)))))
ID3605DS4$hopeless<-ifelse(ID3605DS4$hopeless=="ALL of the time",4,
                  ifelse(ID3605DS4$hopeless=="MOST of the time",3,
                  ifelse(ID3605DS4$hopeless=="SOME of the time",2,
                  ifelse(ID3605DS4$hopeless=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$hopeless=="NONE of the time",0,NA)))))
ID3605DS4$effort<-ifelse(ID3605DS4$effort=="ALL of the time",4,
                  ifelse(ID3605DS4$effort=="MOST of the time",3,
                  ifelse(ID3605DS4$effort=="SOME of the time",2,
                  ifelse(ID3605DS4$effort=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$effort=="NONE of the time",0,NA)))))
ID3605DS4$worthls<-ifelse(ID3605DS4$worthls=="ALL of the time",4,
                  ifelse(ID3605DS4$worthls=="MOST of the time",3,
                  ifelse(ID3605DS4$worthls=="SOME of the time",2,
                  ifelse(ID3605DS4$worthls=="A LITTLE of the time",1,
                  ifelse(ID3605DS4$worthls=="NONE of the time",0,NA)))))
...
dsitems <- ID3605DS4[c(2:7)]
#dsitemspw <- ID3605DS4[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID3605DS4$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID3605DS4$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID3605DS4$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID3605DS4$LAT<-ifelse(ID3605DS4$hispan_i == "Not Hispanic/Spanish origin", 0, 1)
ID3605DS4$MR<-ifelse(ID3605DS4$rcdt1p_i=="Multiple detailed race*",1,0)
ID3605DS4$WHT<-ifelse(ID3605DS4$rcdt1p_i=="White only",1,0)
ID3605DS4$BLK<-ifelse(ID3605DS4$rcdt1p_i=="Black/African American only",1,0)
ID3605DS4$AI<-ifelse(ID3605DS4$rcdt1p_i=="AIAN only",1,0)
ID3605DS4$AA<-ifelse(((ID3605DS4$rcdt1p_i=="Asian Indian only")|(ID3605DS4$rcdt1p_i=="Chinese only")|
                      (ID3605DS4$rcdt1p_i=="Filipino only")|(ID3605DS4$rcdt1p_i=="Other Asian only")),1,0)
ID3605DS4$OTH<-ifelse(ID3605DS4$rcdt1p_i=="Other race only",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
#ID3605DS4$MR<-ifelse((rowSums(x=cbind(ID3605DS4$WHT,ID3605DS4$BLK,ID3605DS4$AI,ID3605DS4$AA,ID3605DS4$OTH),
#                             na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
ID3605DS4$WHT<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 0)&(ID3605DS4$WHT == 1)), 1, 0)
ID3605DS4$BLK<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 0)&(ID3605DS4$BLK == 1)), 1, 0)
ID3605DS4$AI<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 0)&(ID3605DS4$AI == 1)), 1, 0)
ID3605DS4$AA<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 0)&(ID3605DS4$AA == 1)), 1, 0)
ID3605DS4$OTH<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 0)&(ID3605DS4$OTH == 1)), 1, 0)
ID3605DS4$MR<-ifelse(((ID3605DS4$LAT == 0)&(ID3605DS4$MR == 1)), 1, 0)

#Recode factor into numeric,
ID3605DS4$SEX<-ifelse(ID3605DS4$sex == "Male", 1, 0)

#Recode age 
levels(ID3605DS4$age_p)[levels(ID3605DS4$age_p)=="85+ years"]<-"85"
ID3605DS4$AGECALC<-as.numeric(as.character(ID3605DS4$age_p))

#Income, Education, Occupation ----
##Income
#Take highest income reported
ID3605DS4$INC<-ifelse(ID3605DS4$fincgrp=="0-$4999",2499,                   
               ifelse(ID3605DS4$fincgrp=="$5000-$9999",7499,                 
               ifelse(ID3605DS4$fincgrp=="$10000-$14999",12499,               
               ifelse(ID3605DS4$fincgrp=="$15000-$19999",17499,               
               ifelse(ID3605DS4$fincgrp=="$20000-$24999",22499,              
               ifelse(ID3605DS4$fincgrp=="$25000-$34999",29999,               
               ifelse(ID3605DS4$fincgrp=="$35000-$44999",39999,               
               ifelse(ID3605DS4$fincgrp=="$45000-$54999",49999,               
               ifelse(ID3605DS4$fincgrp=="$55000-$64999",59999,               
               ifelse(ID3605DS4$fincgrp=="$65000-$74999",69999,              
               ifelse(ID3605DS4$fincgrp=="$75000 and over",75100,NA))))))))))) 
               
##Education
#categories are close to continuous 
ID3605DS4$EDU<-ifelse(ID3605DS4$educ=="Never attended/ kindergarten only",0,       
               ifelse(ID3605DS4$educ=="1",1,
               ifelse(ID3605DS4$educ=="2",2, 
               ifelse(ID3605DS4$educ=="3",3,                                       
               ifelse(ID3605DS4$educ=="4",4,                                       
               ifelse(ID3605DS4$educ=="5",5,                                       
               ifelse(ID3605DS4$educ=="6",6,                                       
               ifelse(ID3605DS4$educ=="7",7,                                       
               ifelse(ID3605DS4$educ=="8",8,                                       
               ifelse(ID3605DS4$educ=="9",9,  
               ifelse(ID3605DS4$educ=="10",10,                                      
               ifelse(ID3605DS4$educ=="11",11,                                      
               ifelse(ID3605DS4$educ=="12th grade, no diploma",11.5,                  
               ifelse(ID3605DS4$educ=="HIGH SCHOOL GRADUATE",12,                    
               ifelse(ID3605DS4$educ=="GED or equivalent",12,                      
               ifelse(ID3605DS4$educ=="Some college, no degree",13,                 
               ifelse(ID3605DS4$educ=="AA degree: technical or vocational",14,      
               ifelse(ID3605DS4$educ=="AA degree: academic program",14,             
               ifelse(ID3605DS4$educ=="Bachelor's degree (BA, AB, BS, BBA)",16,     
               ifelse(ID3605DS4$educ=="Master's degree (MA, MS, MEng, MEd, MBA)",18,
               ifelse(ID3605DS4$educ=="Professional degree (MD, DDS, DVM, JD)",18,  
               ifelse(ID3605DS4$educ=="Doctoral degree (PhD, EdD)",22,NA))))))))))))))))))))))
...
write.csv(ID3605DS4, file = 'ID3605DS4_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symtpoms items
ID3605DS4 <- ID3605DS4[which(ID3605DS4$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID3605DS4$AGECALC)
dstatsdep<-describe(ID3605DS4$DEP)
dstatsachv<-NA#describe(ID3605DS4$ACHV)
dstatsinc<-describe(ID3605DS4$INC)
dstatsedu<-describe(ID3605DS4$EDU)
dstatsoccs<-NA#describe(ID3605DS4$OCCS)
dstatsoccp<-NA#describe(ID3605DS4$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID3605DS4_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID3605DS4_dephist.pdf')
hist(ID3605DS4$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID3605DS4_achvhist.pdf')
#hist(ID3605DS4$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID3605DS4_inchist.pdf')
hist(ID3605DS4$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID3605DS4_educhist.pdf')
hist(ID3605DS4$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID3605DS4_occshist.pdf')
#hist(ID3605DS4$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID3605DS4_occphist.pdf')
#hist(ID3605DS4$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
#CH<-ID3605DS4[which((ID3605DS4$AGECALC >= 5)&(ID3605DS4$AGECALC <= 11)), ] 
#AD<-ID3605DS4[which((ID3605DS4$AGECALC >= 12)&(ID3605DS4$AGECALC <= 17)), ]
YD<-ID3605DS4[which((ID3605DS4$AGECALC >= 18)&(ID3605DS4$AGECALC <= 25)), ]
AT<-ID3605DS4[which((ID3605DS4$AGECALC >= 26)&(ID3605DS4$AGECALC <= 64)), ]
OA<-ID3605DS4[which(ID3605DS4$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID3605DS4, then DATAID = 21600)
#DATAID<-'3605'
#names(DATAID)<-'DATAID'
...
ID3605DS4_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID3605DS4_aggregated, file = 'ID3605DS4_aggregated.csv',row.names = FALSE)
