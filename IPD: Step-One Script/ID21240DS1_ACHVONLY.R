...
#Coded variables
#Participant ID: NA
#Measure Name (not a variable in the dataset): NA
#Depressive symptom items: NA 
...
ID21240DS1<-data.frame(YELSTGRD,NEWRACE2,IRSEX,AGE2,IRFAMIN3)
detach(da21240.0001)
...
#Assign dataframe that contains all depressive symptoms (using the column numbers of the items)
#dsitems <- ID21240DS1[c(:)]
...
ID21240DS1$DEP<-NA#rowMeans(x=dsitems,na.rm=TRUE)
...
##Income
ID21240DS1$INC<-ifelse(ID21240DS1$IRFAMIN3=="(1) Less than $10,000 (Including Loss)",900,
...
write.csv(ID21240DS1, file = 'ID21240DS1_ACHV_recoded.csv')

#Descriptives and Correlations -----
#Only AD answered achievement questions, exclude participants without sufficient data
achievementdata<-ID21240DS1[which(ID21240DS1$ACHV>=1.00),]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(achievementdata$AGECALC)
dstatsdep<-NA#describe(achievementdata$DEP)
dstatsachv<-describe(achievementdata$ACHV)
dstatsinc<-describe(achievementdata$INC)
dstatsedu<-NA#describe(achievementdata$EDU)
dstatsoccs<-NA#describe(achievementdata$OCCS)
dstatsoccp<-NA#describe(achievementdata$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID21240DS1_ACHV_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
#pdf('ID21240DS1_ACHV_dephist.pdf')
#hist(achievementdata$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
#dev.off()
pdf('ID21240DS1_ACHV_achvhist.pdf')
hist(achievementdata$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
dev.off()
pdf('ID21240DS1_ACHV_inchist.pdf')
hist(achievementdata$INC, xlab="Income", main = NULL, col="grey")
dev.off()
#pdf('ID21240DS1_ACHV_educhist.pdf')
#hist(achievementdata$EDU, xlab="Educational Attainment", main = NULL, col="grey")
#dev.off()
#pdf('ID21240DS1_ACHV_occshist.pdf')
#hist(achievementdata$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID21240DS1_ACHV_occphist.pdf')
#hist(achievementdata$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(achievementdata$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID21240DS1[which((achievementdata$AGECALC >= 5)&(achievementdata$AGECALC <= 11)), ] 
AD<-ID21240DS1[which((achievementdata$AGECALC >= 12)&(achievementdata$AGECALC <= 17)), ]
#YD<-ID21240DS1[which((achievementdata$AGECALC >= 18)&(achievementdata$AGECALC <= 25)), ]
#AT<-ID21240DS1[which((achievementdata$AGECALC >= 26)&(achievementdata$AGECALC <= 64)), ]
#OA<-ID21240DS1[which(achievementdata$AGECALC >= 65), ]
...
#ALPHAOV <- NA#ci.reliability(data=WHTCHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTCHF[1:6], type = "omega", interval.type = "none")
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTCHM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTCHM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=WHTADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=WHTADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=WHTADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=WHTADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTOAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTOAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=WHTOAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=WHTOAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#names(DATAYEAR)<-'DATAYEAR'
##Compute reliability estimates
#ALPHAOV <- NA#ci.reliability(data=LATCHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATCHF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATCHM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATCHM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
names(DATAYEAR)<-'DATAYEAR'
#Compute reliability estimates
ALPHAOV <- NA#ci.reliability(data=LATADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=LATADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=LATADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=LATADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATOAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATOAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=LATOAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=LATOAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKCHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKCHF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKCHM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKCHM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=BLKADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=BLKADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=BLKADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=BLKADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKOAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKOAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=BLKOAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=BLKOAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AACHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AACHF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AACHMC[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AACHMC[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAOAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAOAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AAOAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AAOAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AICHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AICHF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AICHM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AICHM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AIADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AIADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AIADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AIADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIOAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIOAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=AIOAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=AIOAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRCHF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRCHF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRCHM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRCHM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=MRADF[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=MRADF[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=MRADM[1:6], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=MRADM[1:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRYDF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRYDF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRYDM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRYDM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRATF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRATF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MRATM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MRATM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MROAF[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MROAF[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- NA#ci.reliability(data=MROAM[1:6], type = "alpha", interval.type = "none")
#OMEGAOV <- NA#ci.reliability(data=MROAM[1:6], type = "omega", interval.type = "none")
#ALPHA <- NA#ALPHAOV$est
#OMEGA <- NA#OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID21240DS1_ACHV_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID21240DS1_ACHV_aggregated, file = 'ID21240DS1_ACHV_aggregated.csv',row.names = FALSE)
