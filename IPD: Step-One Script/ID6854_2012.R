#load dataset
library('foreign')
#H12B_R.SAV (demographics) is partial data (a lot missing), merge with prior wave 
#merge with recoded data from 2010 (includes prior wave dep symptoms, race/ethnicity, and education)
demographics<-read.spss('H12B_R.SAV') 
demographicspw<-read.csv(file = 'ID6854DS0_2010_recoded.csv')
...
depression<-read.spss('H12D_R.SAV')
income<-read.spss('H12Q_H.sav')
employment<-read.spss('H12J_R.SAV')
other<-read.spss('H12A_R.SAV')
preload<-read.spss('H12PR_R.SAV')
#Merge datasets by Case ID (V1)
ID68542012<-merge(depression,income,by="HHID",all=TRUE)
ID68542012$HHIDPN<-paste(ID68542012$HHID,ID68542012$PN,sep = "")
ID68542012<-merge(ID68542012,demographics,by="HHIDPN",all=TRUE)
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68542012<-merge(ID68542012,employment,by="HHIDPN",all=TRUE)
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68542012<-merge(ID68542012,other,by="HHIDPN",all=TRUE)
preload$HHIDPN<-paste(preload$HHID,preload$PN,sep = "")
ID68542012<-merge(ID68542012,preload,by="HHIDPN",all=TRUE)
#Coded variables
#Participant ID: HHID (household ID); PN (rep ID)
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#ND110,ND111,ND112,ND113(RC),ND114,ND115(RC),ND116,ND117
#Depressive symptom items prior wave:
#MD110,MD111,MD112,MD113(RC),MD114,MD115(RC),MD116,MD117
#Academic Achievement: NA
#Academic Achievement prior wave: NA
#Ethnicity: NB028 prior wave: MB028
#Race: NB089M1M,NB089M2M,NB089M3M prior wave: MB089M1M,MB089M2M,MB089M3M 
#White: NA
#Black: NA 
...
#Birth Month: NX004_R
#Birth Day: NA
#Evaluation Year: NA501
#Evaluation Month: NA500
#Evaluation Day: NA
#Participant Income: NQ020  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: NQ020+NQ045(partner)
#Participant Education: NB014 prior wave: EDU
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: NJ062M 
...
ID6854DS0<-data.frame(HHIDPN,ND110,ND111,ND112,ND113,ND114,ND115,ND116,ND117,
                      MD110,MD111,MD112,MD113,MD114,MD115,MD116,MD117,NB028,MB028,
                      NB089M1M,NB089M2M,NB089M3M,MB089M1M,MB089M2M,MB089M3M,NX060_R,
                      NA019,NX067_R,NX004_R,NA501,NA500,NQ020,NQ045,NB014,EDU,NJ062M)
detach(ID68542012)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68542012,demographics,demographicspw,depression,employment,income,other,preload)

#Check to make sure there is no funny business
head(ID6854DS0)

#Depressive Symptoms ---------
ID6854DS0$ND110<-ifelse(ID6854DS0$ND110==1,1,
                ifelse(ID6854DS0$ND110==5,0,NA))
ID6854DS0$ND111<-ifelse(ID6854DS0$ND111==1,1,
                ifelse(ID6854DS0$ND111==5,0,NA))                 
ID6854DS0$ND112<-ifelse(ID6854DS0$ND112==1,1,
                ifelse(ID6854DS0$ND112==5,0,NA))
ID6854DS0$ND113<-ifelse(ID6854DS0$ND113==1,0,
                ifelse(ID6854DS0$ND113==5,1,NA))#RC
ID6854DS0$ND114<-ifelse(ID6854DS0$ND114==1,1,
                ifelse(ID6854DS0$ND114==5,0,NA))
ID6854DS0$ND115<-ifelse(ID6854DS0$ND115==1,0,
                ifelse(ID6854DS0$ND115==5,1,NA))#RC
ID6854DS0$ND116<-ifelse(ID6854DS0$ND116==1,1,
                ifelse(ID6854DS0$ND116==5,0,NA))
ID6854DS0$ND117<-ifelse(ID6854DS0$ND117==1,1,
                 ifelse(ID6854DS0$ND117==5,0,NA))
...
#1=YES
#5=NO
ID6854DS0$NB028<-ifelse(is.na(ID6854DS0$NB028),ID6854DS0$MB028,ID6854DS0$NB028)
ID6854DS0$LAT<-ifelse(ID6854DS0$NB028==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#97=OTHER (SPECIFY);
#98=DK (don't know);
#99=RF (refused)
ID6854DS0$NB089M1M<-ifelse(is.na(ID6854DS0$NB089M1M),ID6854DS0$MB089M1M,ID6854DS0$NB089M1M)
ID6854DS0$NB089M2M<-ifelse(is.na(ID6854DS0$NB089M2M),ID6854DS0$MB089M2M,ID6854DS0$NB089M2M)
ID6854DS0$NB089M3M<-ifelse(is.na(ID6854DS0$NB089M3M),ID6854DS0$MB089M3M,ID6854DS0$NB089M3M)
ID6854DS0$BLK<-ifelse(((ID6854DS0$NB089M1M==2)|(ID6854DS0$NB089M2M==2)|(ID6854DS0$NB089M3M==2)),1,0)
ID6854DS0$OTH<-ifelse(((ID6854DS0$NB089M1M==97)|(ID6854DS0$NB089M2M==97)|(ID6854DS0$NB089M3M==97)),1,0)
ID6854DS0$WHT<-ifelse(((ID6854DS0$NB089M1M==1)|(ID6854DS0$NB089M2M==1)|(ID6854DS0$NB089M3M==1)),1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$NX060_R==1,1,
               ifelse(ID6854DS0$NX060_R==2,0,NA))

#Recode age 
ID6854DS0$AGECALC<-ID6854DS0$NA019

#Income, Education, Occupation ----
##Income
ID6854DS0$NQ020[ID6854DS0$NQ020=="99999998"]<-NA
ID6854DS0$NQ020[ID6854DS0$NQ020=="99999999"]<-NA
ID6854DS0$NQ045[ID6854DS0$NQ045=="99999998"]<-NA
ID6854DS0$NQ045[ID6854DS0$NQ045=="99999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$NQ020,ID6854DS0$NQ045),na.rm = TRUE)
        
##Education
ID6854DS0$NB014[ID6854DS0$NB014=="97"]<-NA
ID6854DS0$NB014[ID6854DS0$NB014=="98"]<-NA
ID6854DS0$EDU<-pmax(ID6854DS0$EDU,ID6854DS0$NB014,na.rm=TRUE)
...
write.csv(ID6854DS0, file = 'ID6854DS0_2012_recoded.csv')
...
ID6854DS0_2012_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_2012_aggregated, file = 'ID6854DS0_2012_aggregated.csv',row.names = FALSE)
