...
#Participant Occupation: WRKOCCU2 
...
ID3903DS1<-data.frame(DSTNRVOS,DSTHOPLS,DSTRSTLS,DSTCHEER,DSTEFFRT,DSTNGOOD,YELSTGRD,
                      NEWRACE2,IRSEX,AGE2,IRPINC3,IRFAMIN3,IREDUC2,WRKOCCU2)
detach(da03903.0001)
ID3903DS1$ORIGOBS<-nrow(ID3903DS1)
rm(da03903.0001)
...
ID3903DS1$DSTNRVOS<-ifelse(ID3903DS1$DSTNRVOS=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTNRVOS=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTNRVOS=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTNRVOS=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTNRVOS=="(5) None of the time",0,NA)))))
ID3903DS1$DSTHOPLS<-ifelse(ID3903DS1$DSTHOPLS=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTHOPLS=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTHOPLS=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTHOPLS=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTHOPLS=="(5) None of the time",0,NA)))))
ID3903DS1$DSTRSTLS<-ifelse(ID3903DS1$DSTRSTLS=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTRSTLS=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTRSTLS=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTRSTLS=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTRSTLS=="(5) None of the time",0,NA)))))        
ID3903DS1$DSTCHEER<-ifelse(ID3903DS1$DSTCHEER=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTCHEER=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTCHEER=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTCHEER=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTCHEER=="(5) None of the time",0,NA)))))
ID3903DS1$DSTEFFRT<-ifelse(ID3903DS1$DSTEFFRT=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTEFFRT=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTEFFRT=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTEFFRT=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTEFFRT=="(5) None of the time",0,NA)))))        
ID3903DS1$DSTNGOOD<-ifelse(ID3903DS1$DSTNGOOD=="(1) All of the time",4,
                    ifelse(ID3903DS1$DSTNGOOD=="(2) Most of the time",3,
                    ifelse(ID3903DS1$DSTNGOOD=="(3) Some of the time",2,
                    ifelse(ID3903DS1$DSTNGOOD=="(4) A little of the time",1,
                    ifelse(ID3903DS1$DSTNGOOD=="(5) None of the time",0,NA)))))
...
dsitems <- ID3903DS1[c(1:6)]
#dsitemspw <- ID3903DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID3903DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID3903DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID3903DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)

#Academic Achievement -----
#Recode Academic Achievement 
ID3903DS1$ACHV<-ifelse(ID3903DS1$YELSTGRD=="(1) An 'A+', 'A' or 'A-minus' average",4.0,
                ifelse(ID3903DS1$YELSTGRD=="(2) A 'B+', 'B' or 'B-minus' average",3.0,      
                ifelse(ID3903DS1$YELSTGRD=="(3) A 'C+', 'C' or 'C-minus' average",2.0,
                ifelse(ID3903DS1$YELSTGRD=="(4) A 'D' or less than a 'D' average",1.0,NA)))) 
...
ID3903DS1$LAT<-ifelse(ID3903DS1$NEWRACE2 == "(7) Hispanic", 1, 0)
ID3903DS1$WHT<-ifelse(ID3903DS1$NEWRACE2 == "(1) NonHisp White", 1, 0)
ID3903DS1$BLK<-ifelse(ID3903DS1$NEWRACE2 == "(2) NonHisp Black/Afr Am", 1, 0)
ID3903DS1$AI<-ifelse(ID3903DS1$NEWRACE2 == "(3) NonHisp Native Am/AK Native", 1, 0)
ID3903DS1$AA<-ifelse(((ID3903DS1$NEWRACE2 == "(5) NonHisp Asian")|
                      (ID3903DS1$NEWRACE2 == "(4) NonHisp Native HI/Other Pac Isl")), 1, 0)
ID3903DS1$MR<-ifelse(ID3903DS1$NEWRACE2 == "(6) NonHisp more than one race", 1, 0)
ID3903DS1$OTH<-NA

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
#ID3903DS1$MR<-ifelse((rowSums(x=cbind(ID3903DS1$WHT,ID3903DS1$BLK,ID3903DS1$AI,ID3903DS1$AA,ID3903DS1$OTH),
#                              na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
#ID3903DS1$WHT<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 0)&(ID3903DS1$WHT == 1)), 1, 0)
#ID3903DS1$BLK<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 0)&(ID3903DS1$BLK == 1)), 1, 0)
#ID3903DS1$AI<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 0)&(ID3903DS1$AI == 1)), 1, 0)
#ID3903DS1$AA<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 0)&(ID3903DS1$AA == 1)), 1, 0)
#ID3903DS1$OTH<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 0)&(ID3903DS1$OTH == 1)), 1, 0)
#ID3903DS1$MR<-ifelse(((ID3903DS1$LAT == 0)&(ID3903DS1$MR == 1)), 1, 0)

#Recode factor into numeric,
ID3903DS1$SEX<-ifelse(ID3903DS1$IRSEX == "(1) Male", 1, 0)

#Recode age 
ID3903DS1$AGECALC<-ifelse(ID3903DS1$AGE2=="(01) Respondent is 12 years old",12,               
                   ifelse(ID3903DS1$AGE2=="(02) Respondent is 13 years old",13,               
                   ifelse(ID3903DS1$AGE2=="(03) Respondent is 14 years old",14,               
                   ifelse(ID3903DS1$AGE2=="(04) Respondent is 15 years old",15,               
                   ifelse(ID3903DS1$AGE2=="(05) Respondent is 16 years old",16,               
                   ifelse(ID3903DS1$AGE2=="(06) Respondent is 17 years old",17,               
                   ifelse(ID3903DS1$AGE2=="(07) Respondent is 18 years old",18,               
                   ifelse(ID3903DS1$AGE2=="(08) Respondent is 19 years old",19,               
                   ifelse(ID3903DS1$AGE2=="(09) Respondent is 20 years old",20,               
                   ifelse(ID3903DS1$AGE2=="(10) Respondent is 21 years old",21,               
                   ifelse(ID3903DS1$AGE2=="(11) Respondent is 22 or 23 years old",22.5,         
                   ifelse(ID3903DS1$AGE2=="(12) Respondent is 24 or 25 years old",24.5,         
                   ifelse(ID3903DS1$AGE2=="(13) Respondent is between 26 and 29 years old",27.5,
                   ifelse(ID3903DS1$AGE2=="(14) Respondent is between 30 and 34 years old",32,
                   ifelse(ID3903DS1$AGE2=="(15) Respondent is between 35 and 49 years old",42,
                   ifelse(ID3903DS1$AGE2=="(16) Respondent is between 50 and 64 years old",57,
                   ifelse(ID3903DS1$AGE2=="(17) Respondent is 65 years old or older",65,NA)))))))))))))))))    

#Income, Education, Occupation ----
##Income
ID3903DS1$IRPINC3<-ifelse(ID3903DS1$IRPINC3=="(1) Less than $10,000 (Including Loss)",900,
                   ifelse(ID3903DS1$IRPINC3=="(2) $10,000 - $19,999",14999,                 
                   ifelse(ID3903DS1$IRPINC3=="(3) $20,000 - $29,999",24999,                 
                   ifelse(ID3903DS1$IRPINC3=="(4) $30,000 - $39,999",34999,                 
                   ifelse(ID3903DS1$IRPINC3=="(5) $40,000 - $49,999",44999,                 
                   ifelse(ID3903DS1$IRPINC3=="(6) $50,000 - $74,999",62499,                 
                   ifelse(ID3903DS1$IRPINC3=="(7) $75,000 or more",75000,NA)))))))   
ID3903DS1$IRFAMIN3<-ifelse(ID3903DS1$IRFAMIN3=="(1) Less than $10,000 (Including Loss)",900,
                    ifelse(ID3903DS1$IRFAMIN3=="(2) $10,000 - $19,999",14999,                 
                    ifelse(ID3903DS1$IRFAMIN3=="(3) $20,000 - $29,999",24999,                 
                    ifelse(ID3903DS1$IRFAMIN3=="(4) $30,000 - $39,999",34999,                 
                    ifelse(ID3903DS1$IRFAMIN3=="(5) $40,000 - $49,999",44999,                 
                    ifelse(ID3903DS1$IRFAMIN3=="(6) $50,000 - $74,999",62499,                 
                    ifelse(ID3903DS1$IRFAMIN3=="(7) $75,000 or more",75000,NA)))))))   
ID3903DS1$INC<-pmax(ID3903DS1$IRPINC3,ID3903DS1$IRFAMIN3,na.rm = TRUE)
        
##Education
ID3903DS1$EDU<-ifelse(ID3903DS1$IREDUC=="(01) Fifth grade or less",5,  
               ifelse(ID3903DS1$IREDUC=="(02) Sixth grade",6,                                    
               ifelse(ID3903DS1$IREDUC== "(03) Seventh grade",7,                                  
               ifelse(ID3903DS1$IREDUC=="(04) Eighth grade",8,                                   
               ifelse(ID3903DS1$IREDUC=="(05) Ninth grade",9,                                    
               ifelse(ID3903DS1$IREDUC=="(06) Tenth grade",10,                                    
               ifelse(ID3903DS1$IREDUC=="(07) Eleventh grade",11,                                 
               ifelse(ID3903DS1$IREDUC=="(08) Twelfth grade",12,                                  
               ifelse(ID3903DS1$IREDUC=="(09) Freshman/13th year",13,                             
               ifelse(ID3903DS1$IREDUC=="(10) Sophomore/14th year or Junior/15th year",14.5,       
               ifelse(ID3903DS1$IREDUC=="(11) Senior/16th year or GradProf School (or higher)",16,NA)))))))))))
...
write.csv(ID3903DS1, file = 'ID3903DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID3903DS1 <- ID3903DS1[which(ID3903DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID3903DS1$AGECALC)
dstatsdep<-describe(ID3903DS1$DEP)
dstatsachv<-NA#describe(ID3903DS1$ACHV) after exclusion criteria applied above, all NA for ACHV.
dstatsinc<-describe(ID3903DS1$INC)
dstatsedu<-describe(ID3903DS1$EDU)
dstatsoccs<-NA#describe(ID3903DS1$OCCS)
dstatsoccp<-NA#describe(ID3903DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID3903DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID3903DS1_dephist.pdf')
hist(ID3903DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID3903DS1_achvhist.pdf')
#hist(ID3903DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID3903DS1_inchist.pdf')
hist(ID3903DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID3903DS1_educhist.pdf')
hist(ID3903DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID3903DS1_occshist.pdf')
#hist(ID3903DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID3903DS1_occphist.pdf')
#hist(ID3903DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID3903DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID3903DS1[which((ID3903DS1$AGECALC >= 5)&(ID3903DS1$AGECALC <= 11)), ] 
#AD<-ID3903DS1[which((ID3903DS1$AGECALC >= 12)&(ID3903DS1$AGECALC <= 17)), ]
YD<-ID3903DS1[which((ID3903DS1$AGECALC >= 18)&(ID3903DS1$AGECALC <= 25)), ]
AT<-ID3903DS1[which((ID3903DS1$AGECALC >= 26)&(ID3903DS1$AGECALC <= 64)), ]
OA<-ID3903DS1[which(ID3903DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID3903DS1, then DATAID = 21600)
#DATAID<-'3903'
#names(DATAID)<-'DATAID'
...
ID3903DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID3903DS1_aggregated, file = 'ID3903DS1_aggregated.csv',row.names = FALSE)
