...
ID3903DS1<-data.frame(YELSTGRD,NEWRACE2,IRSEX,AGE2,IRFAMIN3)
detach(da03903.0001)
...
ID3903DS1$DEP<-NA#rowMeans(x=dsitems,na.rm=TRUE)
...
##Income
ID3903DS1$INC<-ifelse(ID3903DS1$IRFAMIN3=="(1) Less than $10,000 (Including Loss)",900,
...
write.csv(ID3903DS1, file = 'ID3903DS1_ACHV_recoded.csv')

#Descriptives and Correlations -----
#Only AD answered achievement questions, exclude participants without sufficient data
achievementdata<-ID3903DS1[which(ID3903DS1$ACHV>=1.00),]
...
#CH<-ID3903DS1[which((achievementdata$AGECALC >= 5)&(achievementdata$AGECALC <= 11)), ] 
AD<-ID3903DS1[which((achievementdata$AGECALC >= 12)&(achievementdata$AGECALC <= 17)), ]
#YD<-ID3903DS1[which((achievementdata$AGECALC >= 18)&(achievementdata$AGECALC <= 25)), ]
#AT<-ID3903DS1[which((achievementdata$AGECALC >= 26)&(achievementdata$AGECALC <= 64)), ]
#OA<-ID3903DS1[which(achievementdata$AGECALC >= 65), ]
...
ID3903DS1_ACHV_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID3903DS1_ACHV_aggregated, file = 'ID3903DS1_ACHV_aggregated.csv',row.names = FALSE)
