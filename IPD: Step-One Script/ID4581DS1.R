#Load data
da04581.0001<-as.data.frame(foreign::read.spss('04581-0001-Data.sav'))
da04581.0001<-dplyr::select(da04581.0001,-ubeth,-ubrace,-umkaeth,-umkarace)
#merge with family income data
da04581.0008<-as.data.frame(foreign::read.spss('04581-0008-Data.sav'))
da04581.0008<-dplyr::select(da04581.0008,hhid,ublinc)
merged4581<-merge(da04581.0001,da04581.0008,by="hhid",all.x = TRUE)
#merge with DS2 because for race because umkarace in DS1 is funky
da04581.0002<-as.data.frame(foreign::read.spss('04581-0002-Data.sav'))
da04581.0002<-dplyr::select(da04581.0002,hhid,ubeth,ubrace)
merged4581<-merge(merged4581,da04581.0002,by="hhid",all.x = TRUE)
#This dataset contains data on up to two focal children per adult...
#remove duplicate adult row of dep symptoms (respid), focal child data does not have...
#self-identified race, achievement, or multiple depressive symptom items.
merged4581<-merged4581[!duplicated(merged4581$respid), ]
#Coded variables
#Participant ID: respid
#Measure Name (not a variable in the dataset): SF36
#Depressive symptom items: 
#ndepresa,ndepresb(RC),ndepresc,ndepresd(RC),ndeprese
...
#Maternal Income: NA
#Paternal Income: NA
#Household Income: ublinc
#Participant Education: umedulev
...
#Notes: National Survey of America's Families (NSAF), 1997

#Create Dataset ----
attach(merged4581)
#Extract relevant variables into a dataframe.
ID4581DS1<-data.frame(respid,ndepresa,ndepresb,ndepresc,ndepresd,ndeprese,ubeth,ubrace,
                      umkagend,umkaage,ublinc,umedulev)
detach(merged4581)
ID4581DS1$ORIGOBS<-nrow(ID4581DS1)
rm(da04581.0001,da04581.0008,da04581.0002,merged4581)

#Check to make sure there is no funny business
head(ID4581DS1)

#Depressive Symptoms ---------
ID4581DS1$ndepresa<-dplyr::recode(ID4581DS1$ndepresa,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4581DS1$ndepresb<-dplyr::recode(ID4581DS1$ndepresb,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4581DS1$ndepresc<-dplyr::recode(ID4581DS1$ndepresc,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4581DS1$ndepresd<-dplyr::recode(ID4581DS1$ndepresd,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4581DS1$ndeprese<-dplyr::recode(ID4581DS1$ndeprese,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
...
dsitems <- ID4581DS1[c(2:6)]
#dsitemspw <- ID4581DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID4581DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID4581DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID4581DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID4581DS1$LAT<-ifelse(ID4581DS1$ubeth=="Hispanic",1,0)
ID4581DS1$WHT<-ifelse(ID4581DS1$ubrace=="White",1,0)
ID4581DS1$BLK<-ifelse(ID4581DS1$ubrace=="Black",1,0)
ID4581DS1$AI<-NA
ID4581DS1$AA<-NA
ID4581DS1$OTH<-ifelse(ID4581DS1$ubrace=="Other",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID4581DS1$MR<-NA#ifelse((rowSums(x=cbind(ID4581DS1$WHT,ID4581DS1$BLK,ID4581DS1$AI,ID4581DS1$AA,
#                                       ID4581DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID4581DS1$WHT<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$WHT == 1)), 1, 0)
ID4581DS1$BLK<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$BLK == 1)), 1, 0)
#ID4581DS1$AI<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$AI == 1)), 1, 0)
#ID4581DS1$AA<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$AA == 1)), 1, 0)
ID4581DS1$OTH<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$OTH == 1)), 1, 0)
#ID4581DS1$MR<-ifelse(((ID4581DS1$LAT == 0)&(ID4581DS1$MR == 1)), 1, 0)

#Recode sex
ID4581DS1$SEX<-ifelse(ID4581DS1$umkagend == "Male", 1, 0)

#Recode age 
ID4581DS1$AGECALC<-ID4581DS1$umkaage

#Income, Education, Occupation ----
##Income
ID4581DS1$INC<-ID4581DS1$ublinc

##Education 
ID4581DS1$EDU<-dplyr::recode(ID4581DS1$umedulev,
                             "completed 8th grade"=8,                         
                             "completed 9th or 10th or 11th grade"=10,         
                             "completed 12th grade, no hs diploma or GE"=12,   
                             "GED"=12,                                         
                             "high school diploma"=12,                         
                             "completed some college classes, no college d"=13,
                             "Associate degree"=14,                            
                             "Bachelors degree"=16,                            
                             "cmpltd some grad/prfsnl classes no grad/prfs"=17,
                             "graduate or professional degree"=18)
  
##Occupation
#Nam-Powers-Boyd scale (Nam & Boyd, 2008)
#occupation categories in this dataset are too broad
ID4581DS1$OCCS<-NA
...
write.csv(ID4581DS1, file = 'ID4581DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID4581DS1 <- ID4581DS1[which(ID4581DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID4581DS1$AGECALC)
dstatsdep<-describe(ID4581DS1$DEP)
dstatsachv<-NA#describe(ID4581DS1$ACHV)
dstatsinc<-describe(ID4581DS1$INC)
dstatsedu<-describe(ID4581DS1$EDU)
dstatsoccs<-NA#describe(ID4581DS1$OCCS)
dstatsoccp<-NA#describe(ID4581DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID4581DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID4581DS1_dephist.pdf')
hist(ID4581DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID4581DS1_achvhist.pdf')
#hist(ID4581DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID4581DS1_inchist.pdf')
hist(ID4581DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID4581DS1_educhist.pdf')
hist(ID4581DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID4581DS1_occshist.pdf')
#hist(ID4581DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID4581DS1_occphist.pdf')
#hist(ID4581DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID4581DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID4581DS1[which((ID4581DS1$AGECALC >= 5)&(ID4581DS1$AGECALC <= 11)), ] 
AD<-ID4581DS1[which((ID4581DS1$AGECALC >= 12)&(ID4581DS1$AGECALC <= 17)), ]
YD<-ID4581DS1[which((ID4581DS1$AGECALC >= 18)&(ID4581DS1$AGECALC <= 25)), ]
AT<-ID4581DS1[which((ID4581DS1$AGECALC >= 26)&(ID4581DS1$AGECALC <= 64)), ]
OA<-ID4581DS1[which(ID4581DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID4581DS1, then DATAID = 21600)
#DATAID<-'4581'
#names(DATAID)<-'DATAID'
...
#ALPHAOV <- ci.reliability(data=WHTCHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=WHTCHM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=WHTCHM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTADF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTADF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTADM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTADM[2:6], type = "omega", interval.type = "none")
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTYDM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTYDM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTATM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTATM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=WHTOAM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=WHTOAM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#names(DATAYEAR)<-'DATAYEAR'
##Compute reliability estimates
#ALPHAOV <- ci.reliability(data=LATCHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATCHM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATCHM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
names(DATAYEAR)<-'DATAYEAR'
#Compute reliability estimates
ALPHAOV <- ci.reliability(data=LATADF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATADF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATADM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATADM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATYDF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATYDF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATYDM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATYDM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATATM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATATM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATOAF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATOAF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATOAM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATOAM[2:6], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKCHM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKCHM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKADF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKADF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKADM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKADM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKYDF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKYDF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKYDM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKYDM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKATM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKATM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKOAF[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKOAF[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=BLKOAM[2:6], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=BLKOAM[2:6], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AACHMC[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AACHMC[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAYDF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAYDF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAYDM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAYDM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAATF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAATF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAATM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAATM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAOAM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAOAM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AICHM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AICHM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIADM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIADM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIYDM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIYDM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIATF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIATF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIATM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIATM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AIOAM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AIOAM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRCHM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRCHM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRADM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRADM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRYDM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRYDM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MRATM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MRATM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAF[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAF[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=MROAM[2:6], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=MROAM[2:6], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID4581DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID4581DS1_aggregated, file = 'ID4581DS1_aggregated.csv',row.names = FALSE)