#load dataset
library('foreign')
demographics<-read.spss('H04B_R.SAV')
depression<-read.spss('H04D_R.SAV')
income<-read.spss('H04Q_H.sav')
employment<-read.spss('H04J_R.SAV')
other<-read.spss('H04A_R.SAV')
preload<-read.spss('H04PR_R.SAV')
#Merge datasets by Case ID 
ID68542004<-merge(demographics,income,by="HHID")
ID68542004$HHIDPN<-paste(ID68542004$HHID,ID68542004$PN,sep = "")
depression$HHIDPN<-paste(depression$HHID,depression$PN,sep = "")
ID68542004<-merge(ID68542004,depression,by="HHIDPN")
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68542004<-merge(ID68542004,employment,by="HHIDPN")
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68542004<-merge(ID68542004,other,by="HHIDPN")
preload$HHIDPN<-paste(preload$HHID,preload$PN,sep = "")
ID68542004<-merge(ID68542004,preload,by="HHIDPN")
#Merge prior wave dep symptoms
depressionpw<-read.spss('H02D_R.SAV')
depressionpw$HHIDPN<-paste(depressionpw$HHID,depressionpw$PN,sep = "")
ID68542004<-merge(ID68542004,depressionpw,by="HHIDPN",all.x = TRUE)
#Coded variables
...
#Birth Month: JX004_R
#Birth Day: NA
#Evaluation Year: JA501
#Evaluation Month: JA500
#Evaluation Day: NA
#Participant Income: JQ020  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: JQ020+JQ045(partner)
#Participant Education: JB014A
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: JJ061M 
...
ID6854DS0<-data.frame(HHIDPN,JD110,JD111,JD112,JD113,JD114,JD115,JD116,JD117,
                      HD110,HD111,HD112,HD113,HD114,HD115,HD116,HD117,JB028A,JB031A,
                      JX060_R,JA019,JX067_R,JX004_R,JA501,JA500,JQ020,JQ045,JB014A,JJ061M)

detach(ID68542004)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68542004,demographics,depression,employment,income,other,preload,depressionpw)
...
#1=YES
#5=NO
ID6854DS0$LAT<-ifelse(ID6854DS0$JB028A==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#7=OTHER (SPECIFY);
#8=DK (don't know);
#9=RF (refused)
ID6854DS0$BLK<-ifelse(ID6854DS0$JB031A==2,1,0)
ID6854DS0$OTH<-ifelse(ID6854DS0$JB031A==7,1,0)
ID6854DS0$WHT<-ifelse(ID6854DS0$JB031A==1,1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$JX060_R==1,1,
               ifelse(ID6854DS0$JX060_R==2,0,NA))

#Recode age 
ID6854DS0$AGECALC<-ID6854DS0$JA019

#Income, Education, Occupation ----
##Income
ID6854DS0$JQ020[ID6854DS0$JQ020=="9999998"]<-NA
ID6854DS0$JQ020[ID6854DS0$JQ020=="9999999"]<-NA
ID6854DS0$JQ045[ID6854DS0$JQ045=="999998"]<-NA
ID6854DS0$JQ045[ID6854DS0$JQ045=="999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$JQ020,ID6854DS0$JQ045),na.rm = TRUE)
        
##Education
ID6854DS0$JB014A[ID6854DS0$JB014A=="97"]<-NA
ID6854DS0$JB014A[ID6854DS0$JB014A=="99"]<-NA
ID6854DS0$EDU<-ID6854DS0$JB014A
...
write.csv(ID6854DS0, file = 'ID6854DS0_2004_recoded.csv')
...
ID6854DS0_2004_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_2004_aggregated, file = 'ID6854DS0_2004_aggregated.csv',row.names = FALSE)
