#load dataset
library('foreign')
demographics<-read.spss('H00A_R.SAV')
depression<-read.spss('H00C_R.SAV')
income<-read.spss('H00J_H.sav')
employment<-read.spss('H00G_R.SAV')
other<-read.spss('H00CS_R.SAV')
#Merge datasets by Case ID 
ID68542000<-merge(demographics,income,by="HHID")
ID68542000$HHIDPN<-paste(ID68542000$HHID,ID68542000$PN,sep = "")
depression$HHIDPN<-paste(depression$HHID,depression$PN,sep = "")
ID68542000<-merge(ID68542000,depression,by="HHIDPN")
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68542000<-merge(ID68542000,employment,by="HHIDPN")
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68542000<-merge(ID68542000,other,by="HHIDPN")
#Merge prior wave dep symptoms
depressionpw<-read.spss('H98C_R.SAV')
depressionpw$HHIDPN<-paste(depressionpw$HHID,depressionpw$PN,sep = "")
ID68542000<-merge(ID68542000,depressionpw,by="HHIDPN",all.x = TRUE)
#Coded variables
...
#Birth Month: G1051A
#Birth Day: NA
#Evaluation Year: G770
#Evaluation Month: G768
#Evaluation Day: NA
#Participant Income: G5082  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: G5082+G5115(partner)
#Participant Education: G1074A
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: G3436M 
...
ID6854DS0<-data.frame(HHIDPN,G1669,G1670,G1671,G1672,G1673,G1674,G1675,G1676,
                      F1493,F1494,F1495,F1496,F1497,F1498,F1499,F1500,G1089A,G1092A,
                      G757,G1053A,G1051A,G770,G768,G5082,G5115,G1074A,G3436M)

detach(ID68542000)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68542000,demographics,depression,employment,income,other,depressionpw)
...
#1=YES
#5=NO
ID6854DS0$LAT<-ifelse(ID6854DS0$G1089A==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#7=OTHER (SPECIFY);
#8=DK (don't know);
#9=RF (refused)
ID6854DS0$BLK<-ifelse(ID6854DS0$G1092A==2,1,0)
ID6854DS0$OTH<-ifelse(ID6854DS0$G1092A==7,1,0)
ID6854DS0$WHT<-ifelse(ID6854DS0$G1092A==1,1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$G757==1,1,
               ifelse(ID6854DS0$G757==2,0,NA))

#Recode age 
ID6854DS0$AGECALC<-ID6854DS0$G770-ID6854DS0$G1053A

#Income, Education, Occupation ----
##Income
ID6854DS0$G5082[ID6854DS0$G5082=="99999998"]<-NA
ID6854DS0$G5082[ID6854DS0$G5082=="99999999"]<-NA
ID6854DS0$G5115[ID6854DS0$G5115=="99999998"]<-NA
ID6854DS0$G5115[ID6854DS0$G5115=="99999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$G5082,ID6854DS0$G5115),na.rm = TRUE)
        
##Education
ID6854DS0$G1074A[ID6854DS0$G1074A=="97"]<-NA
ID6854DS0$G1074A[ID6854DS0$G1074A=="98"]<-NA
ID6854DS0$EDU<-ID6854DS0$G1074A
...
write.csv(ID6854DS0, file = 'ID6854DS0_2000_recoded.csv')
...
ID6854DS0_2000_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_2000_aggregated, file = 'ID6854DS0_2000_aggregated.csv',row.names = FALSE)
