#load dataset
library('foreign')
#H18B_R.SAV (demographics) is partial data (a lot missing), merge with prior wave 
#merge with recoded data from 2016 (includes prior wave dep symptoms, race/ethnicity, and education)
demographics<-read.spss('H18B_R.SAV') 
demographicspw<-read.csv(file = 'ID6854DS0_2016_recoded.csv')
...
depression<-read.spss('H18D_R.SAV')
income<-read.spss('H18Q_H.sav')
employment<-read.spss('H18J_R.SAV')
other<-read.spss('H18A_R.SAV')
preload<-read.spss('H18PR_R.SAV')
#Merge datasets by Case ID (V1)
ID68542018<-merge(depression,income,by="HHID",all=TRUE)
ID68542018$HHIDPN<-paste(ID68542018$HHID,ID68542018$PN,sep = "")
ID68542018<-merge(ID68542018,demographics,by="HHIDPN",all=TRUE)
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68542018<-merge(ID68542018,employment,by="HHIDPN",all=TRUE)
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68542018<-merge(ID68542018,other,by="HHIDPN",all=TRUE)
preload$HHIDPN<-paste(preload$HHID,preload$PN,sep = "")
ID68542018<-merge(ID68542018,preload,by="HHIDPN",all=TRUE)
#Coded variables
#Participant ID: HHID (household ID); PN (rep ID)
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#QD110,QD111,QD112,QD113(RC),QD114,QD115(RC),QD116,QD117
...
#Ethnicity: QB028 prior wave: PB028
#Race: QB089M1M,QB089M2M,QB089M3M prior wave: PB089M1M,PB089M2M,PB089M3M 
#White: NA
#Black: NA 
...
#Birth Month: QX004_R
#Birth Day: NA
#Evaluation Year: QA501
#Evaluation Month: QA500
#Evaluation Day: NA
#Participant Income: QQ020  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: QQ020+QQ045(partner)
#Participant Education: QB014 prior wave: EDU
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: QJ168M 
...
ID6854DS0<-data.frame(HHIDPN,QD110,QD111,QD112,QD113,QD114,QD115,QD116,QD117,
                      PD110,PD111,PD112,PD113,PD114,PD115,PD116,PD117,QB028,PB028,
                      QB089M1M,QB089M2M,QB089M3M,PB089M1M,PB089M2M,PB089M3M,QX060_R,
                      QA019,QA019,QX004_R,QA501,QA500,QQ020,QQ045,QB014,EDU,QJ168M)
detach(ID68542018)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68542018,demographics,demographicspw,depression,employment,income,other,preload)
...
#9=RF (refused)
ID6854DS0$QD110<-ifelse(ID6854DS0$QD110==1,1,
                ifelse(ID6854DS0$QD110==5,0,NA))
ID6854DS0$QD111<-ifelse(ID6854DS0$QD111==1,1,
                ifelse(ID6854DS0$QD111==5,0,NA))                 
ID6854DS0$QD112<-ifelse(ID6854DS0$QD112==1,1,
                ifelse(ID6854DS0$QD112==5,0,NA))
ID6854DS0$QD113<-ifelse(ID6854DS0$QD113==1,0,
                ifelse(ID6854DS0$QD113==5,1,NA))#RC
ID6854DS0$QD114<-ifelse(ID6854DS0$QD114==1,1,
                ifelse(ID6854DS0$QD114==5,0,NA))
ID6854DS0$QD115<-ifelse(ID6854DS0$QD115==1,0,
                ifelse(ID6854DS0$QD115==5,1,NA))#RC
ID6854DS0$QD116<-ifelse(ID6854DS0$QD116==1,1,
                ifelse(ID6854DS0$QD116==5,0,NA))
ID6854DS0$QD117<-ifelse(ID6854DS0$QD117==1,1,
                 ifelse(ID6854DS0$QD117==5,0,NA))
...
#1=YES
#5=NO
ID6854DS0$QB028<-ifelse(is.na(ID6854DS0$QB028),ID6854DS0$PB028,ID6854DS0$QB028)
ID6854DS0$LAT<-ifelse(ID6854DS0$QB028==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#97=OTHER (SPECIFY);
#98=DK (don't know);
#99=RF (refused)
ID6854DS0$QB089M1M<-ifelse(is.na(ID6854DS0$QB089M1M),ID6854DS0$PB089M1M,ID6854DS0$QB089M1M)
ID6854DS0$QB089M2M<-ifelse(is.na(ID6854DS0$QB089M2M),ID6854DS0$PB089M2M,ID6854DS0$QB089M2M)
ID6854DS0$QB089M3M<-ifelse(is.na(ID6854DS0$QB089M3M),ID6854DS0$PB089M3M,ID6854DS0$QB089M3M)
ID6854DS0$BLK<-ifelse(((ID6854DS0$QB089M1M==2)|(ID6854DS0$QB089M2M==2)|(ID6854DS0$QB089M3M==2)),1,0)
ID6854DS0$OTH<-ifelse(((ID6854DS0$QB089M1M==97)|(ID6854DS0$QB089M2M==97)|(ID6854DS0$QB089M3M==97)),1,0)
ID6854DS0$WHT<-ifelse(((ID6854DS0$QB089M1M==1)|(ID6854DS0$QB089M2M==1)|(ID6854DS0$QB089M3M==1)),1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$QX060_R==1,1,
               ifelse(ID6854DS0$QX060_R==2,0,NA))

#Recode age 
ID6854DS0$AGECALC<-ID6854DS0$QA019

#Income, Education, Occupation ----
##Income
ID6854DS0$QQ020[ID6854DS0$QQ020=="999998"]<-NA
ID6854DS0$QQ020[ID6854DS0$QQ020=="999999"]<-NA
ID6854DS0$QQ045[ID6854DS0$QQ045=="9999998"]<-NA
ID6854DS0$QQ045[ID6854DS0$QQ045=="9999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$QQ020,ID6854DS0$QQ045),na.rm = TRUE)
        
##Education
ID6854DS0$QB014[ID6854DS0$QB014=="98"]<-NA
ID6854DS0$EDU<-pmax(ID6854DS0$EDU,ID6854DS0$QB014,na.rm=TRUE)
...
write.csv(ID6854DS0, file = 'ID6854DS0_2018_recoded.csv')
...
ID6854DS0_2018_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_2018_aggregated, file = 'ID6854DS0_2018_aggregated.csv',row.names = FALSE)
