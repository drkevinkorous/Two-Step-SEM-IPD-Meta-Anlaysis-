#Load data
da03927.0004<-as.data.frame(foreign::read.spss('03927-0004-Data.sav'))
#merge with family income data and education data
da03927.0008<-as.data.frame(foreign::read.spss('03927-0008-Data.sav'))
da03927.0008<-dplyr::select(da03927.0008,hhid,ublinc)
merged3927<-merge(da03927.0004,da03927.0008,by="hhid",all.x = TRUE)
da03927.0002<-as.data.frame(foreign::read.spss('03927-0002-Data.sav'))
da03927.0002<-dplyr::select(da03927.0002,hhid,uedulevl)
merged3927<-merge(merged3927,da03927.0002,by="hhid",all.x = TRUE)
...
merged3927<-merged3927[!duplicated(merged3927$respid), ]
#Coded variables
...
#Participant Education: uedulevl
...
#Notes: National Survey of America's Families (NSAF), 1999

#Create Dataset ----
attach(merged3927)
#Extract relevant variables into a dataframe.
ID3927DS4<-data.frame(respid,ndepresa,ndepresb,ndepresc,ndepresd,ndeprese,ubeth,ubrace,
                      sex,uage,ublinc,uedulevl)
detach(merged3927)
ID3927DS4$ORIGOBS<-nrow(ID3927DS4)
rm(da03927.0004,da03927.0008,da03927.0002,merged3927)

#Check to make sure there is no funny business
head(ID3927DS4)

#Depressive Symptoms ---------
ID3927DS4$ndepresa<-dplyr::recode(ID3927DS4$ndepresa,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID3927DS4$ndepresb<-dplyr::recode(ID3927DS4$ndepresb,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID3927DS4$ndepresc<-dplyr::recode(ID3927DS4$ndepresc,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID3927DS4$ndepresd<-dplyr::recode(ID3927DS4$ndepresd,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID3927DS4$ndeprese<-dplyr::recode(ID3927DS4$ndeprese,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
...
dsitems <- ID3927DS4[c(2:6)]
#dsitemspw <- ID3927DS4[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID3927DS4$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID3927DS4$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID3927DS4$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID3927DS4$LAT<-ifelse(ID3927DS4$ubeth=="Hispanic",1,0)
ID3927DS4$WHT<-ifelse(ID3927DS4$ubrace=="White_duplicated_W",1,0)
ID3927DS4$BLK<-ifelse(ID3927DS4$ubrace=="Black_duplicated_B",1,0)
ID3927DS4$AI<-NA
ID3927DS4$AA<-NA
ID3927DS4$OTH<-ifelse(ID3927DS4$ubrace=="Other_duplicated_O",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID3927DS4$MR<-NA#ifelse((rowSums(x=cbind(ID3927DS4$WHT,ID3927DS4$BLK,ID3927DS4$AI,ID3927DS4$AA,
#                                       ID3927DS4$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID3927DS4$WHT<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$WHT == 1)), 1, 0)
ID3927DS4$BLK<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$BLK == 1)), 1, 0)
#ID3927DS4$AI<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$AI == 1)), 1, 0)
#ID3927DS4$AA<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$AA == 1)), 1, 0)
ID3927DS4$OTH<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$OTH == 1)), 1, 0)
#ID3927DS4$MR<-ifelse(((ID3927DS4$LAT == 0)&(ID3927DS4$MR == 1)), 1, 0)

#Recode sex
ID3927DS4$SEX<-ifelse(ID3927DS4$sex == "Male", 1, 0)

#Recode age 
ID3927DS4$AGECALC<-ID3927DS4$uage

#Income, Education, Occupation ----
##Income
ID3927DS4$INC<-ID3927DS4$ublinc

##Education 
ID3927DS4$EDU<-dplyr::recode(ID3927DS4$uedulevl,
...
write.csv(ID3927DS4, file = 'ID3927DS4_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID3927DS4 <- ID3927DS4[which(ID3927DS4$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID3927DS4$AGECALC)
dstatsdep<-describe(ID3927DS4$DEP)
dstatsachv<-NA#describe(ID3927DS4$ACHV)
dstatsinc<-describe(ID3927DS4$INC)
dstatsedu<-describe(ID3927DS4$EDU)
dstatsoccs<-NA#describe(ID3927DS4$OCCS)
dstatsoccp<-NA#describe(ID3927DS4$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID3927DS4_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID3927DS4_dephist.pdf')
hist(ID3927DS4$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID3927DS4_achvhist.pdf')
#hist(ID3927DS4$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID3927DS4_inchist.pdf')
hist(ID3927DS4$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID3927DS4_educhist.pdf')
hist(ID3927DS4$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID3927DS4_occshist.pdf')
#hist(ID3927DS4$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID3927DS4_occphist.pdf')
#hist(ID3927DS4$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID3927DS4$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID3927DS4[which((ID3927DS4$AGECALC >= 5)&(ID3927DS4$AGECALC <= 11)), ] 
#AD<-ID3927DS4[which((ID3927DS4$AGECALC >= 12)&(ID3927DS4$AGECALC <= 17)), ]
YD<-ID3927DS4[which((ID3927DS4$AGECALC >= 18)&(ID3927DS4$AGECALC <= 25)), ]
AT<-ID3927DS4[which((ID3927DS4$AGECALC >= 26)&(ID3927DS4$AGECALC <= 64)), ]
#OA<-ID3927DS4[which(ID3927DS4$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID3927DS4, then DATAID = 21600)
#DATAID<-'3927'
#names(DATAID)<-'DATAID'
...
ID3927DS4_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID3927DS4_aggregated, file = 'ID3927DS4_aggregated.csv',row.names = FALSE)