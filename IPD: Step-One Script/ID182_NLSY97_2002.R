...
#Depressive symptom items: S0920800,S0920900(RC),S0921000,S0921100(RC),S0921200
#Depressive symptom items prior wave: R4893600,R4893700(RC),R4893800,R4893900(RC),R4894000
#Academic Achievement: self-report college GPA only this wave
...
#Evaluation Month: NA
#Evaluation Day: NA
#Participant Income: S1055800 
#Maternal Income: NA
#Paternal Income: NA
#Household Income: S1541700 
#Participant Education: S0202000 
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: S1603000 (2002 census codes) 
...
ID182DS0<-data.frame(R0000100,S0920800,S0920900,S0921000,S0921100,S0921200,R4893600,R4893700,R4893800,R4893900,R4894000,
                     S0438700,S0438800,S0438900,S0439000,S0439100,R4281100,R4281200,R4281300,R4281400,R4281500,R4281600,
                     R4281700,R4281800,R4281900,R4282000,R4282100,R4282200,R4282300,R4282400,R4282500,R4282600,
                     S1224800,S1224900,S1224901,S1224902,S1224903,S1224904,S1224905,R0536300,
                     S1531400,R0536402,R0536401,S1055800,S1541700,S0202000,S1603000)
detach(merged182)
...
ID182DS0$ACHV<-rowMeans(x=cbind(ID182DS0$S0438700,ID182DS0$S0438800,ID182DS0$S0438900,
...
ID182DS0$ACHVPW<-rowMeans(x=cbind(ID182DS0$R4281100,ID182DS0$R4281200,ID182DS0$R4281300,ID182DS0$R4281400,
...
#Recode age 
ID182DS0$AGECALC<-ID182DS0$S1531400

#Income, Education, Occupation ----
##Income
#Take highest income reported
ID182DS0$INC<-pmax(ID182DS0$S1055800,ID182DS0$S1541700,na.rm = TRUE)

##Education
ID182DS0$EDU<-ifelse(ID182DS0$S0202000=="1ST GRADE",1,         
              ifelse(ID182DS0$S0202000=="2ND GRADE",2,          
              ifelse(ID182DS0$S0202000=="3RD GRADE",3,         
              ifelse(ID182DS0$S0202000=="4TH GRADE",4,          
              ifelse(ID182DS0$S0202000=="5TH GRADE",5,         
              ifelse(ID182DS0$S0202000=="6TH GRADE",6,          
              ifelse(ID182DS0$S0202000=="7TH GRADE",7,         
              ifelse(ID182DS0$S0202000=="8TH GRADE",8,          
              ifelse(ID182DS0$S0202000=="9TH GRADE",9,         
              ifelse(ID182DS0$S0202000=="10TH GRADE",10,         
              ifelse(ID182DS0$S0202000=="11TH GRADE",11,        
              ifelse(ID182DS0$S0202000=="12TH GRADE",12,         
              ifelse(ID182DS0$S0202000=="1ST YEAR COLLEGE",13,        
              ifelse(ID182DS0$S0202000=="2ND YEAR COLLEGE",14,         
              ifelse(ID182DS0$S0202000=="3RD YEAR COLLEGE",15,        
              ifelse(ID182DS0$S0202000=="4TH YEAR COLLEGE",16,         
              ifelse(ID182DS0$S0202000=="5TH YEAR COLLEGE",17,        
              ifelse(ID182DS0$S0202000=="6TH YEAR COLLEGE",18,         
              ifelse(ID182DS0$S0202000=="7TH YEAR COLLEGE",19,        
              ifelse(ID182DS0$S0202000=="8TH YEAR COLLEGE OR MORE",20,NA))))))))))))))))))))
...
occ_codes<-read.csv('IPD182DS0_NLSY97_2002_Occupational codes.csv')
...
ID182DS0$S1603000<-factor(ID182DS0$S1603000)
ID182DS0$OCCS<-as.numeric(as.character(occstatusvalues[ match(ID182DS0$S1603000, occvalues) ]))
...
ID182DS0$S1603000<-factor(ID182DS0$S1603000)
ID182DS0$OCCP<-as.numeric(as.character(occprestvalues[ match(ID182DS0$S1603000, occvalues) ]))

#Save Recoded Dataset ------
#Save database as .csv for OSF 
write.csv(ID182DS0, file = 'ID182NLSY97_2002_recoded.csv')
...
#Select variables for correlation matrix
AIYDFCmatrixvars<-matrixvars[-12]
...
write.csv(ID182DS0_aggregated, file = 'ID182NLSY97_2002_aggregated.csv',row.names = FALSE)
