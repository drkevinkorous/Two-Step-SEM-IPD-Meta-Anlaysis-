#Load data
da03927.0001<-as.data.frame(foreign::read.spss('03927-0001-Data.sav'))
...
merged3927<-merge(da03927.0001,da03927.0008,by="hhid",all.x = TRUE)
...
ID3927DS1<-data.frame(respid,ndepresa,ndepresb,ndepresc,ndepresd,ndeprese,umkaeth,umkarace,
                      umkagend,umkaage,ublinc,umedulev)
detach(merged3927)
ID3927DS1$ORIGOBS<-nrow(ID3927DS1)
rm(da03927.0001,da03927.0008,merged3927)

#Check to make sure there is no funny business
head(ID3927DS1)

#Depressive Symptoms ---------
ID3927DS1$ndepresa<-dplyr::recode(ID3927DS1$ndepresa,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID3927DS1$ndepresb<-dplyr::recode(ID3927DS1$ndepresb,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID3927DS1$ndepresc<-dplyr::recode(ID3927DS1$ndepresc,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID3927DS1$ndepresd<-dplyr::recode(ID3927DS1$ndepresd,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID3927DS1$ndeprese<-dplyr::recode(ID3927DS1$ndeprese,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
...
dsitems <- ID3927DS1[c(2:6)]
#dsitemspw <- ID3927DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID3927DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID3927DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID3927DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID3927DS1$LAT<-ifelse(ID3927DS1$umkaeth=="Hispanic",1,0)
ID3927DS1$WHT<-ifelse(ID3927DS1$umkarace=="White",1,0)
ID3927DS1$BLK<-ifelse(ID3927DS1$umkarace=="Black",1,0)
ID3927DS1$AI<-NA
ID3927DS1$AA<-NA
ID3927DS1$OTH<-ifelse(ID3927DS1$umkarace=="Other",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID3927DS1$MR<-NA#ifelse((rowSums(x=cbind(ID3927DS1$WHT,ID3927DS1$BLK,ID3927DS1$AI,ID3927DS1$AA,
#                                       ID3927DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID3927DS1$WHT<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$WHT == 1)), 1, 0)
ID3927DS1$BLK<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$BLK == 1)), 1, 0)
#ID3927DS1$AI<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$AI == 1)), 1, 0)
#ID3927DS1$AA<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$AA == 1)), 1, 0)
ID3927DS1$OTH<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$OTH == 1)), 1, 0)
#ID3927DS1$MR<-ifelse(((ID3927DS1$LAT == 0)&(ID3927DS1$MR == 1)), 1, 0)

#Recode sex
ID3927DS1$SEX<-ifelse(ID3927DS1$umkagend == "Male", 1, 0)

#Recode age 
ID3927DS1$AGECALC<-ID3927DS1$umkaage

#Income, Education, Occupation ----
##Income
ID3927DS1$INC<-ID3927DS1$ublinc

##Education 
ID3927DS1$EDU<-dplyr::recode(ID3927DS1$umedulev,
...
write.csv(ID3927DS1, file = 'ID3927DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID3927DS1 <- ID3927DS1[which(ID3927DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID3927DS1$AGECALC)
dstatsdep<-describe(ID3927DS1$DEP)
dstatsachv<-NA#describe(ID3927DS1$ACHV)
dstatsinc<-describe(ID3927DS1$INC)
dstatsedu<-describe(ID3927DS1$EDU)
dstatsoccs<-NA#describe(ID3927DS1$OCCS)
dstatsoccp<-NA#describe(ID3927DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID3927DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID3927DS1_dephist.pdf')
hist(ID3927DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID3927DS1_achvhist.pdf')
#hist(ID3927DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID3927DS1_inchist.pdf')
hist(ID3927DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID3927DS1_educhist.pdf')
hist(ID3927DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID3927DS1_occshist.pdf')
#hist(ID3927DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID3927DS1_occphist.pdf')
#hist(ID3927DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID3927DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID3927DS1[which((ID3927DS1$AGECALC >= 5)&(ID3927DS1$AGECALC <= 11)), ] 
AD<-ID3927DS1[which((ID3927DS1$AGECALC >= 12)&(ID3927DS1$AGECALC <= 17)), ]
YD<-ID3927DS1[which((ID3927DS1$AGECALC >= 18)&(ID3927DS1$AGECALC <= 25)), ]
AT<-ID3927DS1[which((ID3927DS1$AGECALC >= 26)&(ID3927DS1$AGECALC <= 64)), ]
OA<-ID3927DS1[which(ID3927DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID3927DS1, then DATAID = 21600)
#DATAID<-'3927'
#names(DATAID)<-'DATAID'
...
ID3927DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID3927DS1_aggregated, file = 'ID3927DS1_aggregated.csv',row.names = FALSE)