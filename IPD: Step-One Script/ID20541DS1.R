...
#NOTEAT,FLTDEP,FLTEFF,NOSLEEP,WASHAPY(RC),WASLONLY,UNFRIEND,ENJLIFE(RC),FLTSAD,DISLIKD,NOTGETGO
...
#Maternal Income: NA
#Paternal Income: NA
#Household Income: HEARN_RECODE,IML50K,IML25K,IML100K
#Participant Education: DEGREE_RECODE
...
#Notes: National Social Life, Health, and Aging Project (NSHAP): Wave 1, [United States], July 2005-March 2006

#Create Dataset ----
attach(da20541.0001)
#Extract relevant variables into a dataframe.
ID20541DS1<-data.frame(ID,NOTEAT,FLTDEP,FLTEFF,NOSLEEP,WASHAPY,WASLONLY,UNFRIEND,ENJLIFE,
                      FLTSAD,DISLIKD,NOTGETGO,HISPANIC,RACE_RECODE,GENDER,AGE,HEARN_RECODE,
                      IML50K,IML25K,IML100K,DEGREE_RECODE)
detach(da20541.0001)
ID20541DS1$ORIGOBS<-nrow(ID20541DS1)
rm(da20541.0001)
...
#Depressive Symptoms ---------
ID20541DS1$NOTEAT<-dplyr::recode(ID20541DS1$NOTEAT,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$FLTDEP<-dplyr::recode(ID20541DS1$FLTDEP,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$FLTEFF<-dplyr::recode(ID20541DS1$FLTEFF,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$NOSLEEP<-dplyr::recode(ID20541DS1$NOSLEEP,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$WASHAPY<-dplyr::recode(ID20541DS1$WASHAPY,
                                 "(1) rarely or none of the time"=3,
                                 "(2) some of the time"=2,          
                                 "(3) occasionally"=1,               
                                 "(4) most of the time"=0)#RC
ID20541DS1$WASLONLY<-dplyr::recode(ID20541DS1$WASLONLY,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$UNFRIEND<-dplyr::recode(ID20541DS1$UNFRIEND,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$ENJLIFE<-dplyr::recode(ID20541DS1$ENJLIFE,
                                 "(1) rarely or none of the time"=3,
                                 "(2) some of the time"=2,          
                                 "(3) occasionally"=1,               
                                 "(4) most of the time"=0)#RC
ID20541DS1$FLTSAD<-dplyr::recode(ID20541DS1$FLTSAD,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$DISLIKD<-dplyr::recode(ID20541DS1$DISLIKD,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
ID20541DS1$NOTGETGO<-dplyr::recode(ID20541DS1$NOTGETGO,
                                 "(1) rarely or none of the time"=0,
                                 "(2) some of the time"=1,          
                                 "(3) occasionally"=2,               
                                 "(4) most of the time"=3)
...
dsitems <- ID20541DS1[c(2:12)]
#dsitemspw <- ID20541DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID20541DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID20541DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID20541DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID20541DS1$ACHVPW<-NA

#Demographics ----
#Recode race/ethnicity
ID20541DS1$LAT<-ifelse(ID20541DS1$HISPANIC=="(1) yes",1,0)
ID20541DS1$WHT<-ifelse(ID20541DS1$RACE_RECODE=="(1) white/caucasian",1,0)
ID20541DS1$BLK<-ifelse(ID20541DS1$RACE_RECODE=="(2) black/african american",1,0)
ID20541DS1$AI<-NA
ID20541DS1$AA<-NA
ID20541DS1$OTH<-ifelse(ID20541DS1$RACE_RECODE=="(3) asian, pacific islander, american indian or alaskan native",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID20541DS1$MR<-NA#ifelse((rowSums(x=cbind(ID20541DS1$WHT,ID20541DS1$BLK,ID20541DS1$AI,ID20541DS1$AA,
#                                       ID20541DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID20541DS1$WHT<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$WHT == 1)), 1, 0)
ID20541DS1$BLK<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$BLK == 1)), 1, 0)
#ID20541DS1$AI<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$AI == 1)), 1, 0)
#ID20541DS1$AA<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$AA == 1)), 1, 0)
ID20541DS1$OTH<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$OTH == 1)), 1, 0)
#ID20541DS1$MR<-ifelse(((ID20541DS1$LAT == 0)&(ID20541DS1$MR == 1)), 1, 0)

#Recode sex
ID20541DS1$SEX<-ifelse(ID20541DS1$GENDER == "(1) male", 1, 0)

#Recode age 
ID20541DS1$AGECALC<-ID20541DS1$AGE

#Income, Education, Occupation ----
##Income
ID20541DS1$HEARN_RECODE<-dplyr::recode(ID20541DS1$HEARN_RECODE,
                                       "(1) 0-24,999"=12499.5,              
                                       "(2) 25,000-49,999"=37499.5,        
                                       "(3) 50,000-99,999"=73999.5,         
                                       "(4) 100k or higher"=100000)
ID20541DS1$IML50K<-dplyr::recode(ID20541DS1$IML50K,
                                 #"(1) less than $50,000" go to IML25K
                                 "(2) about $50,000"=50000)   
                                 #"(3) more than $50,000" go to IML100K
ID20541DS1$IML25K<-dplyr::recode(ID20541DS1$IML25K,
                                 "(1) less than $25,000"=24999, 
                                 "(2) about $25,000"=25000,     
                                 "(3) more than $25,000"=37500) #$25,000 - $50,000
ID20541DS1$IML100K<-dplyr::recode(ID20541DS1$IML100K,
                                  "(1) less than $100,000"=75000, #$50,000 - $100,000
                                  "(2) about $100,000"=100000,     
                                  "(3) more than $100,000"=101000)
ID20541DS1$INC<-pmax(ID20541DS1$HEARN_RECODE,ID20541DS1$IML50K,ID20541DS1$IML25K,ID20541DS1$IML100K,na.rm = TRUE)

##Education 
ID20541DS1$EDU<-dplyr::recode(ID20541DS1$DEGREE_RECODE,
                              "(1) none"=0,                           
                              "(2) high school diploma/equivalency"=12, 
                              "(3) associates"=14,                     
                              "(4) bachelors"=16,                       
                              "(5) masters"=18,                        
                              "(6) law, md or phd"=20)                  
...
write.csv(ID20541DS1, file = 'ID20541DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID20541DS1 <- ID20541DS1[which(ID20541DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID20541DS1$AGECALC)
dstatsdep<-describe(ID20541DS1$DEP)
dstatsachv<-NA#describe(ID20541DS1$ACHV)
dstatsinc<-describe(ID20541DS1$INC)
dstatsedu<-describe(ID20541DS1$EDU)
dstatsoccs<-NA#describe(ID20541DS1$OCCS)
dstatsoccp<-NA#describe(ID20541DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID20541DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID20541DS1_dephist.pdf')
hist(ID20541DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID20541DS1_achvhist.pdf')
#hist(ID20541DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID20541DS1_inchist.pdf')
hist(ID20541DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID20541DS1_educhist.pdf')
hist(ID20541DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID20541DS1_occshist.pdf')
#hist(ID20541DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID20541DS1_occphist.pdf')
#hist(ID20541DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID20541DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID20541DS1[which((ID20541DS1$AGECALC >= 5)&(ID20541DS1$AGECALC <= 11)), ] 
#AD<-ID20541DS1[which((ID20541DS1$AGECALC >= 12)&(ID20541DS1$AGECALC <= 17)), ]
#YD<-ID20541DS1[which((ID20541DS1$AGECALC >= 18)&(ID20541DS1$AGECALC <= 25)), ]
AT<-ID20541DS1[which((ID20541DS1$AGECALC >= 26)&(ID20541DS1$AGECALC <= 64)), ]
OA<-ID20541DS1[which(ID20541DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID20541DS1, then DATAID = 21600)
#DATAID<-'20541'
#names(DATAID)<-'DATAID'
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADM[2:12], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADM[2:12], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID20541DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID20541DS1_aggregated, file = 'ID20541DS1_aggregated.csv',row.names = FALSE)