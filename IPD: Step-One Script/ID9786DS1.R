#Load data
library('foreign')
da09786.0001<-read.spss('09786-0001-Data.sav') #Data was downloaded in ASCII format and setup in SPSS
#Coded variables
#Participant ID: NA
...
#Household Education: ED_ADULT
#Participant Occupation: NA 
#Maternal Occupation: NA
#Paternal Occupation: NA
#Household Occupation: NA
#SES: NA
#Year of data collection (not a variable in the dataset): 1989
#Notes: Teenage Attitudes and Practices Survey, 1989: [United States]

#Create Dataset ----
attach(da09786.0001)
#Extract relevant variables into a dataframe.
ID9786DS1<-data.frame(Q48A,Q48B,Q48C,Q48D,Q48E,Q48F,SP_HISPO,SP_RACE,SEX,AGE,DOB_YR,DOB_MO,
                      PROC_YR,FAMINC,ED_ADULT)
detach(da09786.0001)
ID9786DS1$ORIGOBS<-nrow(ID9786DS1)
rm(da09786.00017)
...
#Depressive Symptoms ---------
ID9786DS1$Q48A<-ifelse(ID9786DS1$Q48A=="OFTEN",3,
                ifelse(ID9786DS1$Q48A=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48A=="RARELY",1,
                ifelse(ID9786DS1$Q48A=="NEVER",0,NA))))
ID9786DS1$Q48B<-ifelse(ID9786DS1$Q48B=="OFTEN",3,
                ifelse(ID9786DS1$Q48B=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48B=="RARELY",1,
                ifelse(ID9786DS1$Q48B=="NEVER",0,NA))))
ID9786DS1$Q48C<-ifelse(ID9786DS1$Q48C=="OFTEN",3,
                ifelse(ID9786DS1$Q48C=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48C=="RARELY",1,
                ifelse(ID9786DS1$Q48C=="NEVER",0,NA)))) 
ID9786DS1$Q48D<-ifelse(ID9786DS1$Q48D=="OFTEN",3,
                ifelse(ID9786DS1$Q48D=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48D=="RARELY",1,
                ifelse(ID9786DS1$Q48D=="NEVER",0,NA))))   
ID9786DS1$Q48E<-ifelse(ID9786DS1$Q48E=="OFTEN",3,
                ifelse(ID9786DS1$Q48E=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48E=="RARELY",1,
                ifelse(ID9786DS1$Q48E=="NEVER",0,NA)))) 
ID9786DS1$Q48F<-ifelse(ID9786DS1$Q48F=="OFTEN",3,
                ifelse(ID9786DS1$Q48F=="SOMETIMES",2,
                ifelse(ID9786DS1$Q48F=="RARELY",1,
                ifelse(ID9786DS1$Q48F=="NEVER",0,NA))))
...
dsitems <- ID9786DS1[c(1:6)]
#dsitemspw <- ID9786DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID9786DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID9786DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID9786DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID9786DS1$LAT<-ifelse(((ID9786DS1$SP_HISPO=="NOT SPAN ORIGIN")|(ID9786DS1$SP_HISPO=="DK IF SPAN ORIGN")),0,1)
ID9786DS1$WHT<-ifelse(ID9786DS1$SP_RACE=="WHITE",1,0)
ID9786DS1$BLK<-ifelse(ID9786DS1$SP_RACE=="BLACK",1,0)
ID9786DS1$OTH<-ifelse(ID9786DS1$SP_RACE=="OTHER",1,0)
ID9786DS1$AI<-NA
ID9786DS1$AA<-NA

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID9786DS1$MR<- NA#ifelse((rowSums(x=cbind(ID9786DS1$WHT,ID9786DS1$BLK,ID9786DS1$AI,ID9786DS1$AA,
#                                        ID9786DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/HISPANI3)
ID9786DS1$WHT<-ifelse(((ID9786DS1$LAT == 0)&(ID9786DS1$WHT == 1)), 1, 0)
ID9786DS1$BLK<-ifelse(((ID9786DS1$LAT == 0)&(ID9786DS1$BLK == 1)), 1, 0)
#ID9786DS1$AI<-ifelse(((ID9786DS1$LAT == 0)&(ID9786DS1$AI == 1)), 1, 0)
#ID9786DS1$AA<-ifelse(((ID9786DS1$LAT == 0)&(ID9786DS1$AA == 1)), 1, 0)
ID9786DS1$OTH<-ifelse(((ID9786DS1$LAT == 0)&(ID9786DS1$OTH == 1)), 1, 0)


#Recode sex
ID9786DS1$SEX<-ifelse(ID9786DS1$SEX == "MALE", 1, 0)

#Recode age 
ID9786DS1$AGECALC<-ID9786DS1$AGE

#Income, Education, Occupation ----
##Income 
ID9786DS1$INC<-ifelse(ID9786DS1$FAMINC=="LESS THAN 1,000",999, 
                ifelse(ID9786DS1$FAMINC=="1,000-1,999",1499,     
                ifelse(ID9786DS1$FAMINC=="2,000-2,999",2499,    
                ifelse(ID9786DS1$FAMINC=="3,000-3,999",3499,     
                ifelse(ID9786DS1$FAMINC=="4,000-4,999",4499,     
                ifelse(ID9786DS1$FAMINC=="5,000-5,999",5499,    
                ifelse(ID9786DS1$FAMINC=="6,000-6,999",6499,     
                ifelse(ID9786DS1$FAMINC=="7,000-7,999",7499,     
                ifelse(ID9786DS1$FAMINC=="8,000-8,999",8499,    
                ifelse(ID9786DS1$FAMINC=="9,000-9,999",9499,     
                ifelse(ID9786DS1$FAMINC=="10,000-10,999",10499,   
                ifelse(ID9786DS1$FAMINC=="11,000-11,999",11499,  
                ifelse(ID9786DS1$FAMINC=="12,000-12,999",12499,   
                ifelse(ID9786DS1$FAMINC=="13,000-13,999",13499,   
                ifelse(ID9786DS1$FAMINC=="14,000-14,999",14499,  
                ifelse(ID9786DS1$FAMINC=="15,000-15,999",15499,   
                ifelse(ID9786DS1$FAMINC=="16,000-16,999",16499,   
                ifelse(ID9786DS1$FAMINC=="17,000-17,999",17499,  
                ifelse(ID9786DS1$FAMINC=="18,000-18,999",18499,   
                ifelse(ID9786DS1$FAMINC=="19,000-19,999",19499,   
                ifelse(ID9786DS1$FAMINC=="20,000-24,999",22499, 
                ifelse(ID9786DS1$FAMINC=="25,000-29,999",27499,   
                ifelse(ID9786DS1$FAMINC=="30,000-34,999",32499,   
                ifelse(ID9786DS1$FAMINC=="35,000-39,999",37499,  
                ifelse(ID9786DS1$FAMINC=="40,000-44,999",42499,   
                ifelse(ID9786DS1$FAMINC=="45,000-49,999",47499,   
                ifelse(ID9786DS1$FAMINC=="50,000 AND OVER",50000,NA)))))))))))))))))))))))))))

##Education 
ID9786DS1$EDU<-ifelse(ID9786DS1$ED_ADULT=="NONE",0,             
               ifelse(ID9786DS1$ED_ADULT=="1-8 YRS ELEM",4.5,     
               ifelse(ID9786DS1$ED_ADULT=="9-11 YRS HI SCHL",10, 
               ifelse(ID9786DS1$ED_ADULT=="12 YRS H SCH GRD",12,
               ifelse(ID9786DS1$ED_ADULT=="1-3 YRS COLLEGE",14,  
               ifelse(ID9786DS1$ED_ADULT=="4 YRS COLL GRAD",16,  
               ifelse(ID9786DS1$ED_ADULT=="5+ YR POST-COLL",17,NA)))))))
...
write.csv(ID9786DS1, file = 'ID9786DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID9786DS1 <- ID9786DS1[which(ID9786DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID9786DS1$AGECALC)
dstatsdep<-describe(ID9786DS1$DEP)
dstatsachv<-NA#describe(ID9786DS1$ACHV)
dstatsinc<-describe(ID9786DS1$INC)
dstatsedu<-describe(ID9786DS1$EDU)
dstatsoccs<-NA#describe(ID9786DS1$OCCS)
dstatsoccp<-NA#describe(ID9786DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID9786DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID9786DS1_dephist.pdf')
hist(ID9786DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID9786DS1_achvhist.pdf')
#hist(ID9786DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID9786DS1_inchist.pdf')
hist(ID9786DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID9786DS1_educhist.pdf')
hist(ID9786DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID9786DS1_occshist.pdf')
#hist(ID9786DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID9786DS1_occphist.pdf')
#hist(ID9786DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID9786DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID9786DS1[which((ID9786DS1$AGECALC >= 5)&(ID9786DS1$AGECALC <= 11)), ] 
AD<-ID9786DS1[which((ID9786DS1$AGECALC >= 12)&(ID9786DS1$AGECALC <= 17)), ]
YD<-ID9786DS1[which((ID9786DS1$AGECALC >= 18)&(ID9786DS1$AGECALC <= 25)), ]
#AT<-ID9786DS1[which((ID9786DS1$AGECALC >= 26)&(ID9786DS1$AGECALC <= 64)), ]
#OA<-ID9786DS1[which(ID9786DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID9786DS1, then DATAID = 21600)
#DATAID<-'9786'
#names(DATAID)<-'DATAID'
...
##Select variables for correlation matrix
#BLKOAMCmatrixvars<-matrixvars[-c(5:8)]
#BLKOAMC <- BLKOAM[BLKOAMCmatrixvars]
...
ID9786DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID9786DS1_aggregated, file = 'ID9786DS1_aggregated.csv',row.names = FALSE)
