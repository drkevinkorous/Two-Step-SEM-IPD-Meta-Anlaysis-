#load dataset
library('foreign')
da02954.0007<-read.spss('02954-0007-Data.sav') 
#merge with income data
da02954.0002<-read.spss('02954-0002-Data.sav') 
merged2954<-merge(da02954.0007,da02954.0002,by="hhx")
#Coded variables
#Participant ID: hhx
...
#Household Education: fm_educ
#Participant Occupation: occup1 
#Maternal Occupation: NA
#Paternal Occupation: NA
#Household Occupation: NA
#SES: NA
#Year of data collection (not a variable in the dataset): 1997
#Notes: NHIS

#Create Dataset ----
attach(merged2954)
#Extract relevant variables into a dataframe.
ID2954DS7<-data.frame(hhx,sad,nervous,restless,hopeless,effort,worthls,hispan_p,mrace_p,
                      sex,age_p,srvy_yr.x,fincgrp,educ,occup1) 
detach(merged2954)
ID2954DS7$ORIGOBS<-nrow(ID2954DS7)
rm(da02954.0002,da02954.0007)
...
ID2954DS7$sad<-ifelse(ID2954DS7$sad=="ALL of the time",4,
                  ifelse(ID2954DS7$sad=="MOST of the time",3,
                  ifelse(ID2954DS7$sad=="SOME of the time",2,
                  ifelse(ID2954DS7$sad=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$sad=="NONE of the time",0,NA)))))
ID2954DS7$nervous<-ifelse(ID2954DS7$nervous=="ALL of the time",4,
                  ifelse(ID2954DS7$nervous=="MOST of the time",3,
                  ifelse(ID2954DS7$nervous=="SOME of the time",2,
                  ifelse(ID2954DS7$nervous=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$nervous=="NONE of the time",0,NA)))))
ID2954DS7$restless<-ifelse(ID2954DS7$restless=="ALL of the time",4,
                  ifelse(ID2954DS7$restless=="MOST of the time",3,
                  ifelse(ID2954DS7$restless=="SOME of the time",2,
                  ifelse(ID2954DS7$restless=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$restless=="NONE of the time",0,NA)))))
ID2954DS7$hopeless<-ifelse(ID2954DS7$hopeless=="ALL of the time",4,
                  ifelse(ID2954DS7$hopeless=="MOST of the time",3,
                  ifelse(ID2954DS7$hopeless=="SOME of the time",2,
                  ifelse(ID2954DS7$hopeless=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$hopeless=="NONE of the time",0,NA)))))
ID2954DS7$effort<-ifelse(ID2954DS7$effort=="ALL of the time",4,
                  ifelse(ID2954DS7$effort=="MOST of the time",3,
                  ifelse(ID2954DS7$effort=="SOME of the time",2,
                  ifelse(ID2954DS7$effort=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$effort=="NONE of the time",0,NA)))))
ID2954DS7$worthls<-ifelse(ID2954DS7$worthls=="ALL of the time",4,
                  ifelse(ID2954DS7$worthls=="MOST of the time",3,
                  ifelse(ID2954DS7$worthls=="SOME of the time",2,
                  ifelse(ID2954DS7$worthls=="A LITTLE of the time",1,
                  ifelse(ID2954DS7$worthls=="NONE of the time",0,NA)))))
...
dsitems <- ID2954DS7[c(2:7)]
#dsitemspw <- ID2954DS7[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID2954DS7$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID2954DS7$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID2954DS7$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID2954DS7$LAT<-ifelse(ID2954DS7$hispan_p == "Not Hispanic/Spanish origin", 0, 1)
ID2954DS7$MR<-ifelse(((ID2954DS7$mrace_p=="Multiple race")&(ID2954DS7$LAT==0)),1,0)
ID2954DS7$WHT<-ifelse(((ID2954DS7$mrace_p=="White")&(ID2954DS7$LAT==0)),1,0)
ID2954DS7$BLK<-ifelse(((ID2954DS7$mrace_p=="Black/African American")&(ID2954DS7$LAT==0)),1,0)
ID2954DS7$AI<-ifelse(((ID2954DS7$mrace_p=="Indian (American) (includes Eskimo, Aleut)")&(ID2954DS7$LAT==0)),1,0)
ID2954DS7$AA<-ifelse(((ID2954DS7$mrace_p=="Asian Indian")|(ID2954DS7$mrace_p=="Chinese")|
                      (ID2954DS7$mrace_p=="Filipino")|(ID2954DS7$mrace_p=="Other API*")),1,0)
ID2954DS7$AA<-ifelse(((ID2954DS7$AA==1)&(ID2954DS7$LAT==0)),1,0)
ID2954DS7$OTH<-ifelse(((ID2954DS7$mrace_p=="Other race")&&(ID2954DS7$LAT==0)),1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
#ID2954DS7$MR<-ifelse((rowSums(x=cbind(ID2954DS7$WHT,ID2954DS7$BLK,ID2954DS7$AI,ID2954DS7$AA,ID2954DS7$OTH),
#                             na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
#ID2954DS7$WHT<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 0)&(ID2954DS7$WHT == 1)), 1, 0)
#ID2954DS7$BLK<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 0)&(ID2954DS7$BLK == 1)), 1, 0)
#ID2954DS7$AI<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 0)&(ID2954DS7$AI == 1)), 1, 0)
#ID2954DS7$AA<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 0)&(ID2954DS7$AA == 1)), 1, 0)
#ID2954DS7$OTH<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 0)&(ID2954DS7$OTH == 1)), 1, 0)
#ID2954DS7$MR<-ifelse(((ID2954DS7$LAT == 0)&(ID2954DS7$MR == 1)), 1, 0)

#Recode factor into numeric (sex),
ID2954DS7$SEX<-ifelse(ID2954DS7$sex == "Male", 1, 0)

#Recode age 
levels(ID2954DS7$age_p)[levels(ID2954DS7$age_p)=="85+ years"]<-"85"
ID2954DS7$AGECALC<-as.numeric(as.character(ID2954DS7$age_p))

#Income, Education, Occupation ----
##Income
#Take highest income reported
ID2954DS7$INC<-ifelse(ID2954DS7$fincgrp=="0-$4999",2499,                   
               ifelse(ID2954DS7$fincgrp=="$5000-$9999",7499,                 
               ifelse(ID2954DS7$fincgrp=="$10000-$14999",12499,               
               ifelse(ID2954DS7$fincgrp=="$15000-$19999",17499,               
               ifelse(ID2954DS7$fincgrp=="$20000-$24999",22499,              
               ifelse(ID2954DS7$fincgrp=="$25000-$34999",29999,               
               ifelse(ID2954DS7$fincgrp=="$35000-$44999",39999,               
               ifelse(ID2954DS7$fincgrp=="$45000-$54999",49999,               
               ifelse(ID2954DS7$fincgrp=="$55000-$64999",59999,               
               ifelse(ID2954DS7$fincgrp=="$65000-$74999",69999,              
               ifelse(ID2954DS7$fincgrp=="$75000 and over",75100,NA))))))))))) 
               
##Education
#categories are close to continuous 
ID2954DS7$EDU<-ifelse(ID2954DS7$educ=="Never attended/ kindergarten only",0,       
               ifelse(ID2954DS7$educ=="1",1,
               ifelse(ID2954DS7$educ=="2",2, 
               ifelse(ID2954DS7$educ=="3",3,                                       
               ifelse(ID2954DS7$educ=="4",4,                                       
               ifelse(ID2954DS7$educ=="5",5,                                       
               ifelse(ID2954DS7$educ=="6",6,                                       
               ifelse(ID2954DS7$educ=="7",7,                                       
               ifelse(ID2954DS7$educ=="8",8,                                       
               ifelse(ID2954DS7$educ=="9",9,  
               ifelse(ID2954DS7$educ=="10",10,                                      
               ifelse(ID2954DS7$educ=="11",11,                                      
               ifelse(ID2954DS7$educ=="12th grade, no diploma",11.5,                  
               ifelse(ID2954DS7$educ=="HIGH SCHOOL GRADUATE",12,                    
               ifelse(ID2954DS7$educ=="GED or equivalent",12,                      
               ifelse(ID2954DS7$educ=="Some college, no degree",13,                 
               ifelse(ID2954DS7$educ=="AA degree: technical or vocational",14,      
               ifelse(ID2954DS7$educ=="AA degree: academic program",14,             
               ifelse(ID2954DS7$educ=="Bachelor's degree (BA, AB, BS, BBA)",16,     
               ifelse(ID2954DS7$educ=="Master's degree (MA, MS, MEng, MEd, MBA)",18,
               ifelse(ID2954DS7$educ=="Professional degree (MD, DDS, DVM, JD)",18,  
               ifelse(ID2954DS7$educ=="Doctoral degree (PhD, EdD)",22,NA))))))))))))))))))))))
...
write.csv(ID2954DS7, file = 'ID2954DS7_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symtpoms items
ID2954DS7 <- ID2954DS7[which(ID2954DS7$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID2954DS7$AGECALC)
dstatsdep<-describe(ID2954DS7$DEP)
dstatsachv<-NA#describe(ID2954DS7$ACHV)
dstatsinc<-describe(ID2954DS7$INC)
dstatsedu<-describe(ID2954DS7$EDU)
dstatsoccs<-NA#describe(ID2954DS7$OCCS)
dstatsoccp<-NA#describe(ID2954DS7$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID2954DS7_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID2954DS7_dephist.pdf')
hist(ID2954DS7$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID2954DS7_achvhist.pdf')
#hist(ID2954DS7$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID2954DS7_inchist.pdf')
hist(ID2954DS7$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID2954DS7_educhist.pdf')
hist(ID2954DS7$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID2954DS7_occshist.pdf')
#hist(ID2954DS7$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID2954DS7_occphist.pdf')
#hist(ID2954DS7$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
#CH<-ID2954DS7[which((ID2954DS7$AGECALC >= 5)&(ID2954DS7$AGECALC <= 11)), ] 
#AD<-ID2954DS7[which((ID2954DS7$AGECALC >= 12)&(ID2954DS7$AGECALC <= 17)), ]
YD<-ID2954DS7[which((ID2954DS7$AGECALC >= 18)&(ID2954DS7$AGECALC <= 25)), ]
AT<-ID2954DS7[which((ID2954DS7$AGECALC >= 26)&(ID2954DS7$AGECALC <= 64)), ]
OA<-ID2954DS7[which(ID2954DS7$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID2954DS7, then DATAID = 21600)
#DATAID<-'2954'
#names(DATAID)<-'DATAID'
...
ID2954DS7_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID2954DS7_aggregated, file = 'ID2954DS7_aggregated.csv',row.names = FALSE)
