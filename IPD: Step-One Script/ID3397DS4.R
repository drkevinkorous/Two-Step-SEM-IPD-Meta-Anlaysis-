#load dataset
library('foreign')
da03397.0004<-read.spss('03397-0004-Data.sav') #Data was downloaded in ASCII format and setup in SPSS
#merge with income data
da03397.0002<-read.spss('03397-0002-Data.sav') #Data was downloaded in ASCII format and setup in SPSS
merged3397<-merge(da03397.0004,da03397.0002,by="HHX")
#Coded variables
#Participant ID: HHX
...
#Birth Month: DOB_M
#Birth Day: NA
#Evaluation Year: SRVY_YR
#Evaluation Month: NA
#Evaluation Day: NA
#Participant Income: NA 
#Maternal Income: NA
#Paternal Income: NA
#Household Income: FINCGRP (DS2) 
#Participant Education: EDUC
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: OCCUP1 
#Maternal Occupation: NA
#Paternal Occupation: NA
#Household Occupation: NA
#SES: NA
#Year of data collection (not a variable in the dataset): 1999
#Notes: NHIS

#Create Dataset ----
attach(merged3397)
#Extract relevant variables into a dataframe.
ID3397DS4<-data.frame(HHX,SAD,NERVOUS,RESTLESS,HOPELESS,EFFORT,WORTHLS,ORIGIN,RACDET_P,
                      SEX,AGE_P,SRVY_YR.x,FINCGRP,EDUC,OCCUP1) 
detach(merged3397)
ID3397DS4$ORIGOBS<-nrow(ID3397DS4)
rm(da03397.0004,da03397.0002)
...
ID3397DS4$SAD<-ifelse(ID3397DS4$SAD=="ALL of the time",4,
                  ifelse(ID3397DS4$SAD=="MOST of the time",3,
                  ifelse(ID3397DS4$SAD=="SOME of the time",2,
                  ifelse(ID3397DS4$SAD=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$SAD=="NONE of the time",0,NA)))))
ID3397DS4$NERVOUS<-ifelse(ID3397DS4$NERVOUS=="ALL of the time",4,
                  ifelse(ID3397DS4$NERVOUS=="MOST of the time",3,
                  ifelse(ID3397DS4$NERVOUS=="SOME of the time",2,
                  ifelse(ID3397DS4$NERVOUS=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$NERVOUS=="NONE of the time",0,NA)))))
ID3397DS4$RESTLESS<-ifelse(ID3397DS4$RESTLESS=="ALL of the time",4,
                  ifelse(ID3397DS4$RESTLESS=="MOST of the time",3,
                  ifelse(ID3397DS4$RESTLESS=="SOME of the time",2,
                  ifelse(ID3397DS4$RESTLESS=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$RESTLESS=="NONE of the time",0,NA)))))
ID3397DS4$HOPELESS<-ifelse(ID3397DS4$HOPELESS=="ALL of the time",4,
                  ifelse(ID3397DS4$HOPELESS=="MOST of the time",3,
                  ifelse(ID3397DS4$HOPELESS=="SOME of the time",2,
                  ifelse(ID3397DS4$HOPELESS=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$HOPELESS=="NONE of the time",0,NA)))))
ID3397DS4$EFFORT<-ifelse(ID3397DS4$EFFORT=="ALL of the time",4,
                  ifelse(ID3397DS4$EFFORT=="MOST of the time",3,
                  ifelse(ID3397DS4$EFFORT=="SOME of the time",2,
                  ifelse(ID3397DS4$EFFORT=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$EFFORT=="NONE of the time",0,NA)))))
ID3397DS4$WORTHLS<-ifelse(ID3397DS4$WORTHLS=="ALL of the time",4,
                  ifelse(ID3397DS4$WORTHLS=="MOST of the time",3,
                  ifelse(ID3397DS4$WORTHLS=="SOME of the time",2,
                  ifelse(ID3397DS4$WORTHLS=="A LITTLE of the time",1,
                  ifelse(ID3397DS4$WORTHLS=="NONE of the time",0,NA)))))
...
dsitems <- ID3397DS4[c(2:7)]
#dsitemspw <- ID3397DS4[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID3397DS4$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID3397DS4$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID3397DS4$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID3397DS4$LAT<-ifelse(ID3397DS4$ORIGIN == "Yes", 1, 0)
ID3397DS4$MR<-ifelse(((ID3397DS4$RACDET_P=="Multiple detailed race*")&(ID3397DS4$LAT==0)),1,0)
ID3397DS4$WHT<-ifelse(((ID3397DS4$RACDET_P=="White only")&(ID3397DS4$LAT==0)),1,0)
ID3397DS4$BLK<-ifelse(((ID3397DS4$RACDET_P=="Black/African American only")&(ID3397DS4$LAT==0)),1,0)
ID3397DS4$AI<-ifelse(((ID3397DS4$RACDET_P=="AIAN only")&(ID3397DS4$LAT==0)),1,0)
ID3397DS4$AA<-ifelse(((ID3397DS4$RACDET_P=="Asian Indian only")|(ID3397DS4$RACDET_P=="Chinese only")|
                      (ID3397DS4$RACDET_P=="Filipino only")|(ID3397DS4$RACDET_P=="Other Asian only")),1,0)
ID3397DS4$AA<-ifelse(((ID3397DS4$AA==1)&(ID3397DS4$LAT==0)),1,0)
ID3397DS4$OTH<-ifelse(((ID3397DS4$RACDET_P=="Other race only")&&(ID3397DS4$LAT==0)),1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
#ID3397DS4$MR<-ifelse((rowSums(x=cbind(ID3397DS4$WHT,ID3397DS4$BLK,ID3397DS4$AI,ID3397DS4$AA,ID3397DS4$OTH),
#                             na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
#ID3397DS4$WHT<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 0)&(ID3397DS4$WHT == 1)), 1, 0)
#ID3397DS4$BLK<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 0)&(ID3397DS4$BLK == 1)), 1, 0)
#ID3397DS4$AI<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 0)&(ID3397DS4$AI == 1)), 1, 0)
#ID3397DS4$AA<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 0)&(ID3397DS4$AA == 1)), 1, 0)
#ID3397DS4$OTH<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 0)&(ID3397DS4$OTH == 1)), 1, 0)
#ID3397DS4$MR<-ifelse(((ID3397DS4$LAT == 0)&(ID3397DS4$MR == 1)), 1, 0)

#Recode factor into numeric (sex),
ID3397DS4$SEX<-ifelse(ID3397DS4$SEX == "Male", 1, 0)

#Recode age 
levels(ID3397DS4$AGE_P)[levels(ID3397DS4$AGE_P)=="85+ years"]<-"85"
ID3397DS4$AGECALC<-as.numeric(as.character(ID3397DS4$AGE_P))

#Income, Education, Occupation ----
##Income
#Take highest income reported
ID3397DS4$INC<-ifelse(ID3397DS4$FINCGRP=="0-$4999",2499,                   
               ifelse(ID3397DS4$FINCGRP=="$5000-$9999",7499,                 
               ifelse(ID3397DS4$FINCGRP=="$10000-$14999",12499,               
               ifelse(ID3397DS4$FINCGRP=="$15000-$19999",17499,               
               ifelse(ID3397DS4$FINCGRP=="$20000-$24999",22499,              
               ifelse(ID3397DS4$FINCGRP=="$25000-$34999",29999,               
               ifelse(ID3397DS4$FINCGRP=="$35000-$44999",39999,               
               ifelse(ID3397DS4$FINCGRP=="$45000-$54999",49999,               
               ifelse(ID3397DS4$FINCGRP=="$55000-$64999",59999,               
               ifelse(ID3397DS4$FINCGRP=="$65000-$74999",69999,              
               ifelse(ID3397DS4$FINCGRP=="$75000 and over",75100,NA))))))))))) 
               
##Education
#categories are close to continuous 
ID3397DS4$EDU<-ifelse(ID3397DS4$EDUC=="Never attended/ kindergarten only",0,       
               ifelse(ID3397DS4$EDUC=="1",1,
               ifelse(ID3397DS4$EDUC=="2",2, 
               ifelse(ID3397DS4$EDUC=="3",3,                                       
               ifelse(ID3397DS4$EDUC=="4",4,                                       
               ifelse(ID3397DS4$EDUC=="5",5,                                       
               ifelse(ID3397DS4$EDUC=="6",6,                                       
               ifelse(ID3397DS4$EDUC=="7",7,                                       
               ifelse(ID3397DS4$EDUC=="8",8,                                       
               ifelse(ID3397DS4$EDUC=="9",9,  
               ifelse(ID3397DS4$EDUC=="10",10,                                      
               ifelse(ID3397DS4$EDUC=="11",11,                                      
               ifelse(ID3397DS4$EDUC=="12th grade, no diploma",11.5,                  
               ifelse(ID3397DS4$EDUC=="HIGH SCHOOL GRADUATE",12,                    
               ifelse(ID3397DS4$EDUC=="GED or equivalent",12,                      
               ifelse(ID3397DS4$EDUC=="Some college, no degree",13,                 
               ifelse(ID3397DS4$EDUC=="AA degree: technical or vocational",14,      
               ifelse(ID3397DS4$EDUC=="AA degree: academic program",14,             
               ifelse(ID3397DS4$EDUC=="Bachelor's degree (BA, AB, BS, BBA)",16,     
               ifelse(ID3397DS4$EDUC=="Master's degree (MA, MS, MEng, MEd, MBA)",18,
               ifelse(ID3397DS4$EDUC=="Professional degree (MD, DDS, DVM, JD)",18,  
               ifelse(ID3397DS4$EDUC=="Doctoral degree (PhD, EdD)",22,NA))))))))))))))))))))))
...
write.csv(ID3397DS4, file = 'ID3397DS4_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symtpoms items
ID3397DS4 <- ID3397DS4[which(ID3397DS4$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID3397DS4$AGECALC)
dstatsdep<-describe(ID3397DS4$DEP)
dstatsachv<-NA#describe(ID3397DS4$ACHV)
dstatsinc<-describe(ID3397DS4$INC)
dstatsedu<-describe(ID3397DS4$EDU)
dstatsoccs<-NA#describe(ID3397DS4$OCCS)
dstatsoccp<-NA#describe(ID3397DS4$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID3397DS4_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID3397DS4_dephist.pdf')
hist(ID3397DS4$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID3397DS4_achvhist.pdf')
#hist(ID3397DS4$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID3397DS4_inchist.pdf')
hist(ID3397DS4$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID3397DS4_educhist.pdf')
hist(ID3397DS4$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID3397DS4_occshist.pdf')
#hist(ID3397DS4$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID3397DS4_occphist.pdf')
#hist(ID3397DS4$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID3397DS4$AGECALC)
#Separate dataset by developmental period, if needed
#although there are ages in the AD range the age variable does not align with variable naming in the study documentation (should have been parent data only). 
#CH<-ID3397DS4[which((ID3397DS4$AGECALC >= 5)&(ID3397DS4$AGECALC <= 11)), ] 
#AD<-ID3397DS4[which((ID3397DS4$AGECALC >= 12)&(ID3397DS4$AGECALC <= 17)), ]
YD<-ID3397DS4[which((ID3397DS4$AGECALC >= 18)&(ID3397DS4$AGECALC <= 25)), ]
AT<-ID3397DS4[which((ID3397DS4$AGECALC >= 26)&(ID3397DS4$AGECALC <= 64)), ]
OA<-ID3397DS4[which(ID3397DS4$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID3397DS4, then DATAID = 21600)
#DATAID<-'3397'
#names(DATAID)<-'DATAID'
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATOAM[2:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATOAM[2:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=AAADMC[2:7], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=AAADMC[2:7], type = "omega", interval.type = "none")
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=AAATM[2:7], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=AAATM[2:7], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AIYDM[2:7], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AIYDM[2:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AIOAM[2:7], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AIOAM[2:7], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID3397DS4_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID3397DS4_aggregated, file = 'ID3397DS4_aggregated.csv',row.names = FALSE)
