#load dataset
library('foreign')
demographics<-read.spss('A95A_R.SAV')
depression<-read.spss('A95B_R.SAV')
income<-read.spss('A95J_H.SAV')
employment<-read.spss('A95G_R.SAV')
other<-read.spss('A95CS_R.SAV')
#Merge datasets
ID68541995<-merge(demographics,income,by="HHID")
ID68541995$HHIDPN<-paste(ID68541995$HHID,ID68541995$PN,sep = "")
depression$HHIDPN<-paste(depression$HHID,depression$PN,sep = "")
ID68541995<-merge(ID68541995,depression,by="HHIDPN")
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68541995<-merge(ID68541995,employment,by="HHIDPN")
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68541995<-merge(ID68541995,other,by="HHIDPN")
#Merge with prior wave dep symptoms 
ds1993<-read.spss('BR21.SAV')
ds1993$HHIDPN<-paste(ds1993$HHID,ds1993$PN,sep = "")
ID68541995<-merge(ID68541995,ds1993,by="HHIDPN",all.x = TRUE)
#Coded variables
#Participant ID: HHID (household ID); PN (rep ID)
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#D985,D987,D989,D991(RC),D993,D995(RC),D999,D1001
...
#Birth Month: D636A
#Birth Day: NA
#Evaluation Year: D397
#Evaluation Month: D398
#Evaluation Day: NA
#Participant Income: NA  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: D4642 
#Participant Education: D649A
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: D2820M 
...
ID6854DS0<-data.frame(HHIDPN,D985,D987,D989,D991,D993,D995,D999,D1001,
                      V307,V309,V310,V311,V312,V314,V315,V316,D664A,D667AM,
                      D374,D638A,D636A,D397,D398,D4642,D649A,D2820M)
detach(ID68541995)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68541995,demographics,depression,ds1993,employment,income,other)
...
#9=RF (refused)
ID6854DS0$D985<-ifelse(ID6854DS0$D985==1,1,
                ifelse(ID6854DS0$D985==5,0,NA))
ID6854DS0$D987<-ifelse(ID6854DS0$D987==1,1,
                ifelse(ID6854DS0$D987==5,0,NA))                 
ID6854DS0$D989<-ifelse(ID6854DS0$D989==1,1,
                ifelse(ID6854DS0$D989==5,0,NA))
ID6854DS0$D991<-ifelse(ID6854DS0$D991==1,0,
                ifelse(ID6854DS0$D991==5,1,NA))#RC
ID6854DS0$D993<-ifelse(ID6854DS0$D993==1,1,
                ifelse(ID6854DS0$D993==5,0,NA))
ID6854DS0$D995<-ifelse(ID6854DS0$D995==1,0,
                ifelse(ID6854DS0$D995==5,1,NA))#RC
ID6854DS0$D999<-ifelse(ID6854DS0$D999==1,1,
                ifelse(ID6854DS0$D999==5,0,NA))
ID6854DS0$D1001<-ifelse(ID6854DS0$D1001==1,1,
                 ifelse(ID6854DS0$D1001==5,0,NA))  
...
#1=YES
#5=NO
ID6854DS0$LAT<-ifelse(ID6854DS0$D664A==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#7=OTHER (SPECIFY);
#8=DK (don't know);
#9=RF (refused)
ID6854DS0$BLK<-ifelse(ID6854DS0$D667AM==2,1,0)
ID6854DS0$OTH<-ifelse(ID6854DS0$D667AM==7,1,0)
ID6854DS0$WHT<-ifelse(ID6854DS0$D667AM==1,1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$D374==1,1,
               ifelse(ID6854DS0$D374==2,0,NA))

#Recode age 
ID6854DS0$D638A[ID6854DS0$D638A=="9999"]<-NA
ID6854DS0$AGECALC<-ID6854DS0$D397-ID6854DS0$D638A

#Income, Education, Occupation ----
##Income
ID6854DS0$D4642[ID6854DS0$D4642=="9999998"]<-NA
ID6854DS0$D4642[ID6854DS0$D4642=="9999999"]<-NA
ID6854DS0$INC<-ID6854DS0$D4642
        
##Education
ID6854DS0$D649A[ID6854DS0$D649A=="97"]<-NA
ID6854DS0$D649A[ID6854DS0$D649A=="98"]<-NA
ID6854DS0$EDU<-ID6854DS0$D649A
...
write.csv(ID6854DS0, file = 'ID6854DS0_1995_recoded.csv')
...
ID6854DS0_1995_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_1995_aggregated, file = 'ID6854DS0_1995_aggregated.csv',row.names = FALSE)
