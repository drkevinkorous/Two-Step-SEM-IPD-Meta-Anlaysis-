#Load data
load(file = '00171-0001-Data.rda')
load(file = '00171-0002-Data.rda')
merged171<-merge(da00171.0001,da00171.0002,by="CASEID1")
names(merged171)[names(merged171)=="CASENUM.x"]<-"MCASEID"
merged171<-merged171[which(merged171$TYPE.x=="R"),]
#Load W1 for race and educaton
load(file = '06041-0001-Data.rda')
merged171<-merge(merged171,da06041.0001,by="MCASEID")
#Load W2 for prior wave dep symptoms
load('06906-0004-Data.rda') #main respondent prior wave dep symptoms
merged171<-merge(merged171,da06906.0004,by="MCASEID",all.x = TRUE)
#Coded variables
#Participant ID: MCASEID, but CASEID1 (includes case number and type [R = respondent, S = spouse])
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#RT206A,RT206B,RT206C,RT206D,RT206E,RT206F,RT206G,RT206H,RT206I,RT206J,RT206K,RT206L
#Depressive symptom items prior wave: 
#MT206A,MT206B,MT206C,MT206D,MT206E,MT206F,MT206G,MT206H,MT206I,MT206J,MT206K,MT206L
...
#Birth Month: DOBM
#Birth Day: NA
#Evaluation Year: IDATYY
#Evaluation Month: IDATMM
#Evaluation Day: IDATDD
#Participant Income: RQ2  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: RQ2,RQ6P01,RQ6P02
#Participant Education: EDUCAT (W1)
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: RP10OCC 
#Maternal Occupation: NA
#Paternal Occupation: NA
#Household Occupation: NA
#SES: NA
#Year of data collection (not a variable in the dataset): 2002 (2001-2003)
#Notes: NSFH Wave 3 (Main Respondent Data) 

#Create Dataset ----
attach(merged171)
#Extract relevant variables into a dataframe.
ID171DS1<-data.frame(MCASEID,RT206A,RT206B,RT206C,RT206D,RT206E,RT206F,RT206G,RT206H,RT206I,RT206J,
                      RT206K,RT206L,MT206A,MT206B,MT206C,MT206D,MT206E,MT206F,MT206G,MT206H,MT206I,
                      MT206J,MT206K,MT206L,M484,SEX_A,DOBY,DOBM,IDATYY,IDATMM,RQ2,RQ6P01,RQ6P02,
                      EDUCAT,RP10OCC) 
detach(merged171)
ID171DS1$ORIGOBS<-nrow(ID171DS1)
rm(merged171,da06041.0001,da06906.0004,da00171.0001,da00171.0002)
...
ID171DS1$RT206A<-ifelse(ID171DS1$RT206A==0,0,
                  ifelse(((ID171DS1$RT206A==1)|(ID171DS1$RT206A==2)),1,
                  ifelse(((ID171DS1$RT206A==3)|(ID171DS1$RT206A==4)),2,
                  ifelse(ID171DS1$RT206A>=5,3,NA))))
ID171DS1$RT206B<-ifelse(ID171DS1$RT206B==0,0,
                  ifelse(((ID171DS1$RT206B==1)|(ID171DS1$RT206B==2)),1,
                  ifelse(((ID171DS1$RT206B==3)|(ID171DS1$RT206B==4)),2,
                  ifelse(ID171DS1$RT206B>=5,3,NA))))
ID171DS1$RT206C<-ifelse(ID171DS1$RT206C==0,0,
                  ifelse(((ID171DS1$RT206C==1)|(ID171DS1$RT206C==2)),1,
                  ifelse(((ID171DS1$RT206C==3)|(ID171DS1$RT206C==4)),2,
                  ifelse(ID171DS1$RT206C>=5,3,NA))))
ID171DS1$RT206D<-ifelse(ID171DS1$RT206D==0,0,
                  ifelse(((ID171DS1$RT206D==1)|(ID171DS1$RT206D==2)),1,
                  ifelse(((ID171DS1$RT206D==3)|(ID171DS1$RT206D==4)),2,
                  ifelse(ID171DS1$RT206D>=5,3,NA))))
ID171DS1$RT206E<-ifelse(ID171DS1$RT206E==0,0,
                  ifelse(((ID171DS1$RT206E==1)|(ID171DS1$RT206E==2)),1,
                  ifelse(((ID171DS1$RT206E==3)|(ID171DS1$RT206E==4)),2,
                  ifelse(ID171DS1$RT206E>=5,3,NA))))
ID171DS1$RT206F<-ifelse(ID171DS1$RT206F==0,0,
                  ifelse(((ID171DS1$RT206F==1)|(ID171DS1$RT206F==2)),1,
                  ifelse(((ID171DS1$RT206F==3)|(ID171DS1$RT206F==4)),2,
                  ifelse(ID171DS1$RT206F>=5,3,NA))))
ID171DS1$RT206G<-ifelse(ID171DS1$RT206G==0,0,
                  ifelse(((ID171DS1$RT206G==1)|(ID171DS1$RT206G==2)),1,
                  ifelse(((ID171DS1$RT206G==3)|(ID171DS1$RT206G==4)),2,
                  ifelse(ID171DS1$RT206G>=5,3,NA))))
ID171DS1$RT206H<-ifelse(ID171DS1$RT206H==0,0,
                  ifelse(((ID171DS1$RT206H==1)|(ID171DS1$RT206H==2)),1,
                  ifelse(((ID171DS1$RT206H==3)|(ID171DS1$RT206H==4)),2,
                  ifelse(ID171DS1$RT206H>=5,3,NA))))
ID171DS1$RT206I<-ifelse(ID171DS1$RT206I==0,0,
                  ifelse(((ID171DS1$RT206I==1)|(ID171DS1$RT206I==2)),1,
                  ifelse(((ID171DS1$RT206I==3)|(ID171DS1$RT206I==4)),2,
                  ifelse(ID171DS1$RT206I>=5,3,NA))))
ID171DS1$RT206J<-ifelse(ID171DS1$RT206J==0,0,
                  ifelse(((ID171DS1$RT206J==1)|(ID171DS1$RT206J==2)),1,
                  ifelse(((ID171DS1$RT206J==3)|(ID171DS1$RT206J==4)),2,
                  ifelse(ID171DS1$RT206J>=5,3,NA))))
ID171DS1$RT206K<-ifelse(ID171DS1$RT206K==0,0,
                  ifelse(((ID171DS1$RT206K==1)|(ID171DS1$RT206K==2)),1,
                  ifelse(((ID171DS1$RT206K==3)|(ID171DS1$RT206K==4)),2,
                  ifelse(ID171DS1$RT206K>=5,3,NA))))
ID171DS1$RT206L<-ifelse(ID171DS1$RT206L==0,0,
                  ifelse(((ID171DS1$RT206L==1)|(ID171DS1$RT206L==2)),1,
                  ifelse(((ID171DS1$RT206L==3)|(ID171DS1$RT206L==4)),2,
                  ifelse(ID171DS1$RT206L>=5,3,NA))))
...
#Occasionally or a moderate amount of time: 3-4 days (code as 2)
#Most or all of the time: 5 - 7 days (Code as 3)
ID171DS1$MT206A<-ifelse(ID171DS1$MT206A==0,0,
                  ifelse(((ID171DS1$MT206A==1)|(ID171DS1$MT206A==2)),1,
                  ifelse(((ID171DS1$MT206A==3)|(ID171DS1$MT206A==4)),2,
                  ifelse(ID171DS1$MT206A>=5,3,NA))))
ID171DS1$MT206B<-ifelse(ID171DS1$MT206B==0,0,
                  ifelse(((ID171DS1$MT206B==1)|(ID171DS1$MT206B==2)),1,
                  ifelse(((ID171DS1$MT206B==3)|(ID171DS1$MT206B==4)),2,
                  ifelse(ID171DS1$MT206B>=5,3,NA))))
ID171DS1$MT206C<-ifelse(ID171DS1$MT206C==0,0,
                  ifelse(((ID171DS1$MT206C==1)|(ID171DS1$MT206C==2)),1,
                  ifelse(((ID171DS1$MT206C==3)|(ID171DS1$MT206C==4)),2,
                  ifelse(ID171DS1$MT206C>=5,3,NA))))
ID171DS1$MT206D<-ifelse(ID171DS1$MT206D==0,0,
                  ifelse(((ID171DS1$MT206D==1)|(ID171DS1$MT206D==2)),1,
                  ifelse(((ID171DS1$MT206D==3)|(ID171DS1$MT206D==4)),2,
                  ifelse(ID171DS1$MT206D>=5,3,NA))))
ID171DS1$MT206E<-ifelse(ID171DS1$MT206E==0,0,
                  ifelse(((ID171DS1$MT206E==1)|(ID171DS1$MT206E==2)),1,
                  ifelse(((ID171DS1$MT206E==3)|(ID171DS1$MT206E==4)),2,
                  ifelse(ID171DS1$MT206E>=5,3,NA))))
ID171DS1$MT206F<-ifelse(ID171DS1$MT206F==0,0,
                  ifelse(((ID171DS1$MT206F==1)|(ID171DS1$MT206F==2)),1,
                  ifelse(((ID171DS1$MT206F==3)|(ID171DS1$MT206F==4)),2,
                  ifelse(ID171DS1$MT206F>=5,3,NA))))
ID171DS1$MT206G<-ifelse(ID171DS1$MT206G==0,0,
                  ifelse(((ID171DS1$MT206G==1)|(ID171DS1$MT206G==2)),1,
                  ifelse(((ID171DS1$MT206G==3)|(ID171DS1$MT206G==4)),2,
                  ifelse(ID171DS1$MT206G>=5,3,NA))))
ID171DS1$MT206H<-ifelse(ID171DS1$MT206H==0,0,
                  ifelse(((ID171DS1$MT206H==1)|(ID171DS1$MT206H==2)),1,
                  ifelse(((ID171DS1$MT206H==3)|(ID171DS1$MT206H==4)),2,
                  ifelse(ID171DS1$MT206H>=5,3,NA))))
ID171DS1$MT206I<-ifelse(ID171DS1$MT206I==0,0,
                  ifelse(((ID171DS1$MT206I==1)|(ID171DS1$MT206I==2)),1,
                  ifelse(((ID171DS1$MT206I==3)|(ID171DS1$MT206I==4)),2,
                  ifelse(ID171DS1$MT206I>=5,3,NA))))
ID171DS1$MT206J<-ifelse(ID171DS1$MT206J==0,0,
                  ifelse(((ID171DS1$MT206J==1)|(ID171DS1$MT206J==2)),1,
                  ifelse(((ID171DS1$MT206J==3)|(ID171DS1$MT206J==4)),2,
                  ifelse(ID171DS1$MT206J>=5,3,NA))))
ID171DS1$MT206K<-ifelse(ID171DS1$MT206K==0,0,
                  ifelse(((ID171DS1$MT206K==1)|(ID171DS1$MT206K==2)),1,
                  ifelse(((ID171DS1$MT206K==3)|(ID171DS1$MT206K==4)),2,
                  ifelse(ID171DS1$MT206K>=5,3,NA))))
ID171DS1$MT206L<-ifelse(ID171DS1$MT206L==0,0,
                  ifelse(((ID171DS1$MT206L==1)|(ID171DS1$MT206L==2)),1,
                  ifelse(((ID171DS1$MT206L==3)|(ID171DS1$MT206L==4)),2,
                  ifelse(ID171DS1$MT206L>=5,3,NA))))

#Assign dataframe that contains all depressive symtpoms (using the column numbers of the items)
dsitems <- ID171DS1[c(2:13)]
dsitemspw <- ID171DS1[c(14:25)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID171DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID171DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID171DS1$DEPPW<-rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID171DS1$LAT<-ifelse(((ID171DS1$M484 == "(3) MEX/CHICANO/MEX.AM")|
                       (ID171DS1$M484 == "(4) PUERTO RICAN")|
                       (ID171DS1$M484 == "(5) CUBAN")|
                       (ID171DS1$M484 == "(6) OTHER HISPANIC")),1,0)
ID171DS1$WHT<-ifelse(ID171DS1$M484=="(2) WHITE/NOT HISPANIC",1,0)
ID171DS1$BLK<-ifelse(ID171DS1$M484=="(1) BLACK",1,0)
ID171DS1$AI<-ifelse(ID171DS1$M484=="(7) AMERICAN INDIAN",1,0)
ID171DS1$AA<-ifelse(ID171DS1$M484=="(8) ASIAN",1,0)
ID171DS1$OTH<-ifelse(ID171DS1$M484=="(9) OTHER",1,0)
#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
ID171DS1$MR<- NA#ifelse((rowSums(x=cbind(ID171DS1$WHT,ID171DS1$BLK,ID171DS1$OTH),
                             #na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
#ID171DS1$WHT<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 0)&(ID171DS1$WHT == 1)), 1, 0)
#ID171DS1$BLK<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 0)&(ID171DS1$BLK == 1)), 1, 0)
#ID171DS1$AI<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 0)&(ID171DS1$AI == 1)), 1, 0)
#ID171DS1$AA<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 0)&(ID171DS1$AA == 1)), 1, 0)
#ID171DS1$OTH<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 0)&(ID171DS1$OTH == 1)), 1, 0)
#ID171DS1$MR<-ifelse(((ID171DS1$LAT == 0)&(ID171DS1$MR == 1)), 1, 0)

#Recode factor into numeric,
ID171DS1$SEX<-ifelse(ID171DS1$SEX_A == "(1) Male", 1, 0)

#Recode age 
ID171DS1$DOBY<-(ID171DS1$DOBY+1900)
ID171DS1$AGECALC<-(ID171DS1$IDATYY-ID171DS1$DOBY)

#Income, Education, Occupation ----
##Income
ID171DS1$INC<-rowSums(cbind(ID171DS1$RQ2T,ID171DS1$RQ6P01,ID171DS1$RQ6P02),na.rm=TRUE)
        
##Education
ID171DS1$EDU<-ifelse(ID171DS1$EDUCAT=="(00) NO FORML EDUCATION",0, 
               ifelse(ID171DS1$EDUCAT=="(01) FIRST GRADE",1,       
               ifelse(ID171DS1$EDUCAT=="(02) SECOND GRADE",2,       
               ifelse(ID171DS1$EDUCAT=="(03) THIRD GRADE",3,       
               ifelse(ID171DS1$EDUCAT=="(04) FOURTH GRADE",4,       
               ifelse(ID171DS1$EDUCAT=="(05) FIFTH GRADE",5,       
               ifelse(ID171DS1$EDUCAT=="(06) SIXTH GRADE",6,        
               ifelse(ID171DS1$EDUCAT=="(07) SEVENTH GRADE",7,     
               ifelse(ID171DS1$EDUCAT=="(08) EIGHT GRADE",8,        
               ifelse(ID171DS1$EDUCAT=="(09) NINTH GRADE",9,       
               ifelse(ID171DS1$EDUCAT=="(10) TENTH GRADE",10,        
               ifelse(ID171DS1$EDUCAT=="(11) ELEVENTH GRADE",11,    
               ifelse(ID171DS1$EDUCAT=="(12) HIGH SCHL GRADUATE",12, 
               ifelse(ID171DS1$EDUCAT=="(13) ATTND COLL/NO DGR",13, 
               ifelse(ID171DS1$EDUCAT=="(14) ASSOC DEGREE-2YR",14,   
               ifelse(ID171DS1$EDUCAT=="(15) ENROLLED 3 YRS",15,    
               ifelse(ID171DS1$EDUCAT=="(16) BACHELOR'S DEGREE",16,  
               ifelse(ID171DS1$EDUCAT=="(17) ENROLLED POST GRAD",17,
               ifelse(ID171DS1$EDUCAT=="(18) MASTER'S DEGREE",18,    
               ifelse(ID171DS1$EDUCAT=="(19) ENROLLD POST-MASTR",19,
               ifelse(ID171DS1$EDUCAT=="(20) DOCTORATE/PROF DGR",20,NA)))))))))))))))))))))

##Occupation
#Recode factor into numeric (occupational status)
#Nam-Powers-Boyd scale (Nam & Boyd, 2008)
occ_codes<-read.csv('ID171DS1_Occupational codes.csv')
...
ID171DS1$OCCS<-as.numeric(as.character(occstatusvalues[ match(ID171DS1$RP10OCC, occvalues) ]))
...
ID171DS1$OCCP<-as.numeric(as.character(occprestvalues[ match(ID171DS1$RP10OCC, occvalues) ]))

#Save Recoded Dataset ------
#Save database as .csv for OSF 
write.csv(ID171DS1, file = 'ID171DS1_R_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID171DS1 <- ID171DS1[which(ID171DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID171DS1$AGECALC)
dstatsdep<-describe(ID171DS1$DEP)
dstatsachv<-NA#describe(ID171DS1$ACHV)
dstatsinc<-describe(ID171DS1$INC)
dstatsedu<-describe(ID171DS1$EDU)
dstatsoccs<-describe(ID171DS1$OCCS)
dstatsoccp<-describe(ID171DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID171DS1_R_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID171DS1_R_dephist.pdf')
hist(ID171DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID171DS1_R_achvhist.pdf')
#hist(ID171DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID171DS1_R_inchist.pdf')
hist(ID171DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID171DS1_R_educhist.pdf')
hist(ID171DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
pdf('ID171DS1_R_occshist.pdf')
hist(ID171DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
dev.off()
pdf('ID171DS1_R_occphist.pdf')
hist(ID171DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
dev.off()
...
table(ID171DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID171DS1[which((ID171DS1$AGECALC >= 5)&(ID171DS1$AGECALC <= 11)), ] 
#AD<-ID171DS1[which((ID171DS1$AGECALC >= 12)&(ID171DS1$AGECALC <= 17)), ]
#YD<-ID171DS1[which((ID171DS1$AGECALC >= 18)&(ID171DS1$AGECALC <= 25)), ]
AT<-ID171DS1[which((ID171DS1$AGECALC >= 26)&(ID171DS1$AGECALC <= 64)), ]
OA<-ID171DS1[which(ID171DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID171DS1, then DATAID = 21600)
#DATAID<-'171'
#names(DATAID)<-'DATAID'
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=LATYDF[2:13], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=LATYDF[2:13], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- ci.reliability(data=LATOAM[2:13], type = "alpha", interval.type = "none")
OMEGAOV <- ci.reliability(data=LATOAM[2:13], type = "omega", interval.type = "none")
ALPHA <- ALPHAOV$est
OMEGA <- OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
##Compute reliability estimates
##Only need to change depressive symptoms items 
#ALPHAOV <- ci.reliability(data=BLKYDF[2:13], type = "alpha", interval.type = "none")
#OMEGAOV <- ci.reliability(data=BLKYDF[2:13], type = "omega", interval.type = "none")
#ALPHA <- ALPHAOV$est
#OMEGA <- OMEGAOV$est
##Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AAATM[2:13], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AAATM[2:13], type = "omega", interval.type = "none")
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- #ci.reliability(data=AIATF[2:13], type = "alpha", interval.type = "none")
OMEGAOV <- #ci.reliability(data=AIATF[2:13], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID171DS1_R_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID171DS1_R_aggregated, file = 'ID171DS1_R_aggregated.csv',row.names = FALSE)
