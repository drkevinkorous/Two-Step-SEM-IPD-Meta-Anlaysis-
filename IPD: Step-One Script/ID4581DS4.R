#Load data
da04581.0004<-as.data.frame(foreign::read.spss('04581-0004-Data.sav'))
...
merged4581<-merge(da04581.0004,da04581.0008,by="hhid",all.x = TRUE)
...
ID4581DS4<-data.frame(respid,ndepresa,ndepresb,ndepresc,ndepresd,ndeprese,ubeth,ubrace,
                      sex,uage,ublinc,uedulevl)
detach(merged4581)
ID4581DS4$ORIGOBS<-nrow(ID4581DS4)
rm(da04581.0004,da04581.0008,merged4581)

#Check to make sure there is no funny business
head(ID4581DS4)

#Depressive Symptoms ---------
ID4581DS4$ndepresa<-dplyr::recode(ID4581DS4$ndepresa,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4581DS4$ndepresb<-dplyr::recode(ID4581DS4$ndepresb,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4581DS4$ndepresc<-dplyr::recode(ID4581DS4$ndepresc,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4581DS4$ndepresd<-dplyr::recode(ID4581DS4$ndepresd,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4581DS4$ndeprese<-dplyr::recode(ID4581DS4$ndeprese,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
...
dsitems <- ID4581DS4[c(2:6)]
#dsitemspw <- ID4581DS4[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID4581DS4$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID4581DS4$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID4581DS4$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID4581DS4$LAT<-ifelse(ID4581DS4$ubeth=="Hispanic",1,0)
ID4581DS4$WHT<-ifelse(ID4581DS4$ubrace=="White",1,0)
ID4581DS4$BLK<-ifelse(ID4581DS4$ubrace=="Black",1,0)
ID4581DS4$AI<-NA
ID4581DS4$AA<-NA
ID4581DS4$OTH<-ifelse(ID4581DS4$ubrace=="Other",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID4581DS4$MR<-NA#ifelse((rowSums(x=cbind(ID4581DS4$WHT,ID4581DS4$BLK,ID4581DS4$AI,ID4581DS4$AA,
#                                       ID4581DS4$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID4581DS4$WHT<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$WHT == 1)), 1, 0)
ID4581DS4$BLK<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$BLK == 1)), 1, 0)
#ID4581DS4$AI<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$AI == 1)), 1, 0)
#ID4581DS4$AA<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$AA == 1)), 1, 0)
ID4581DS4$OTH<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$OTH == 1)), 1, 0)
#ID4581DS4$MR<-ifelse(((ID4581DS4$LAT == 0)&(ID4581DS4$MR == 1)), 1, 0)

#Recode sex
ID4581DS4$SEX<-ifelse(ID4581DS4$sex == "Male", 1, 0)

#Recode age 
ID4581DS4$AGECALC<-ID4581DS4$uage

#Income, Education, Occupation ----
##Income
ID4581DS4$INC<-ID4581DS4$ublinc

##Education 
ID4581DS4$EDU<-dplyr::recode(ID4581DS4$uedulevl,
...
write.csv(ID4581DS4, file = 'ID4581DS4_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID4581DS4 <- ID4581DS4[which(ID4581DS4$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID4581DS4$AGECALC)
dstatsdep<-describe(ID4581DS4$DEP)
dstatsachv<-NA#describe(ID4581DS4$ACHV)
dstatsinc<-describe(ID4581DS4$INC)
dstatsedu<-describe(ID4581DS4$EDU)
dstatsoccs<-NA#describe(ID4581DS4$OCCS)
dstatsoccp<-NA#describe(ID4581DS4$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID4581DS4_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID4581DS4_dephist.pdf')
hist(ID4581DS4$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID4581DS4_achvhist.pdf')
#hist(ID4581DS4$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID4581DS4_inchist.pdf')
hist(ID4581DS4$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID4581DS4_educhist.pdf')
hist(ID4581DS4$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID4581DS4_occshist.pdf')
#hist(ID4581DS4$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID4581DS4_occphist.pdf')
#hist(ID4581DS4$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID4581DS4$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID4581DS4[which((ID4581DS4$AGECALC >= 5)&(ID4581DS4$AGECALC <= 11)), ] 
AD<-ID4581DS4[which((ID4581DS4$AGECALC >= 12)&(ID4581DS4$AGECALC <= 17)), ]
YD<-ID4581DS4[which((ID4581DS4$AGECALC >= 18)&(ID4581DS4$AGECALC <= 25)), ]
AT<-ID4581DS4[which((ID4581DS4$AGECALC >= 26)&(ID4581DS4$AGECALC <= 64)), ]
#OA<-ID4581DS4[which(ID4581DS4$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID4581DS4, then DATAID = 21600)
#DATAID<-'4581'
#names(DATAID)<-'DATAID'
...
ID4581DS4_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID4581DS4_aggregated, file = 'ID4581DS4_aggregated.csv',row.names = FALSE)