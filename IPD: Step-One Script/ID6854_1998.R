#load dataset
library('foreign')
demographics<-read.spss('H98A_R.SAV')
depression<-read.spss('H98C_R.SAV')
income<-read.spss('H98J_H.sav')
employment<-read.spss('H98G_R.SAV')
other<-read.spss('H98CS_R.SAV')
#Merge datasets by Case ID 
ID68541998<-merge(demographics,income,by="HHID")
ID68541998$HHIDPN<-paste(ID68541998$HHID,ID68541998$PN,sep = "")
depression$HHIDPN<-paste(depression$HHID,depression$PN,sep = "")
ID68541998<-merge(ID68541998,depression,by="HHIDPN")
employment$HHIDPN<-paste(employment$HHID,employment$PN,sep = "")
ID68541998<-merge(ID68541998,employment,by="HHIDPN")
other$HHIDPN<-paste(other$HHID,other$PN,sep = "")
ID68541998<-merge(ID68541998,other,by="HHIDPN")
#Merge prior wave dep symptoms
depressionpw<-read.spss('H96B_R.SAV')
depressionpw$HHIDPN<-paste(depressionpw$HHID,depressionpw$PN,sep = "")
ID68541998<-merge(ID68541998,depressionpw,by="HHIDPN",all.x = TRUE)
#Coded variables
#Participant ID: HHID (household ID); PN (rep ID)
#Measure Name (not a variable in the dataset): CESD
#Depressive symptom items: 
#F1493,F1494,F1495,F1496(RC),F1497,F1498(RC),F1499,F1500
#Depressive symptom items prior wave:
#E985,E986,E987,E988(RC),E989,E990(RC),E991,E992
...
#Birth Month: F968A
#Birth Day: NA
#Evaluation Year: F699
#Evaluation Month: F697
#Evaluation Day: NA
#Participant Income: F4643  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: F4643+F4676(partner) 
#Participant Education: F987A
#Maternal Education: NA
#Paternal Education: NA
#Household Education: NA
#Participant Occupation: F3187HM 
...
ID6854DS0<-data.frame(HHIDPN,F1493,F1494,F1495,F1496,F1497,F1498,F1499,F1500,
                      E985,E986,E987,E988,E989,E990,E991,E992,F1002A,F1005A,
                      F686,F970A,F968A,F699,F697,F4643,F4676,F987A,F3187HM)

detach(ID68541998)
ID6854DS0$ORIGOBS<-nrow(ID6854DS0)
rm(ID68541998,demographics,depression,employment,income,other,depressionpw)
...
#9=RF (refused)
ID6854DS0$F1493<-ifelse(ID6854DS0$F1493==1,1,
                ifelse(ID6854DS0$F1493==5,0,NA))
ID6854DS0$F1494<-ifelse(ID6854DS0$F1494==1,1,
                ifelse(ID6854DS0$F1494==5,0,NA))                 
ID6854DS0$F1495<-ifelse(ID6854DS0$F1495==1,1,
                ifelse(ID6854DS0$F1495==5,0,NA))
ID6854DS0$F1496<-ifelse(ID6854DS0$F1496==1,0,
                ifelse(ID6854DS0$F1496==5,1,NA))#RC
ID6854DS0$F1497<-ifelse(ID6854DS0$F1497==1,1,
                ifelse(ID6854DS0$F1497==5,0,NA))
ID6854DS0$F1498<-ifelse(ID6854DS0$F1498==1,0,
                ifelse(ID6854DS0$F1498==5,1,NA))#RC
ID6854DS0$F1499<-ifelse(ID6854DS0$F1499==1,1,
                ifelse(ID6854DS0$F1499==5,0,NA))
ID6854DS0$F1500<-ifelse(ID6854DS0$F1500==1,1,
                 ifelse(ID6854DS0$F1500==5,0,NA))
...
#8=DK (don't know); NA (not ascertained)
#9=RF (refused)
ID6854DS0$E985<-ifelse(ID6854DS0$E985==1,1,
                ifelse(ID6854DS0$E985==5,0,NA))
ID6854DS0$E986<-ifelse(ID6854DS0$E986==1,1,
                ifelse(ID6854DS0$E986==5,0,NA))                 
ID6854DS0$E987<-ifelse(ID6854DS0$E987==1,1,
                ifelse(ID6854DS0$E987==5,0,NA))
ID6854DS0$E988<-ifelse(ID6854DS0$E988==1,0,
                ifelse(ID6854DS0$E988==5,1,NA))#RC
ID6854DS0$E989<-ifelse(ID6854DS0$E989==1,1,
                ifelse(ID6854DS0$E989==5,0,NA))
ID6854DS0$E990<-ifelse(ID6854DS0$E990==1,0,
                ifelse(ID6854DS0$E990==5,1,NA))#RC
ID6854DS0$E991<-ifelse(ID6854DS0$E991==1,1,
                ifelse(ID6854DS0$E991==5,0,NA))
ID6854DS0$E992<-ifelse(ID6854DS0$E992==1,1,
                 ifelse(ID6854DS0$E992==5,0,NA))
...
#1=YES
#5=NO
ID6854DS0$LAT<-ifelse(ID6854DS0$F1002A==1,1,0)
#1=WHITE/CAUCASIAN
#2=BLACK/AFRICAN AM
#7=OTHER (SPECIFY);
#8=DK (don't know);
#9=RF (refused)
ID6854DS0$BLK<-ifelse(ID6854DS0$F1005A==2,1,0)
ID6854DS0$OTH<-ifelse(ID6854DS0$F1005A==7,1,0)
ID6854DS0$WHT<-ifelse(ID6854DS0$F1005A==1,1,0)
...
#1=MALE
#2=FEMALE
ID6854DS0$SEX<-ifelse(ID6854DS0$F686==1,1,
               ifelse(ID6854DS0$F686==2,0,NA))

#Recode age 
ID6854DS0$F970A[ID6854DS0$F970A=="9998"]<-NA
ID6854DS0$F699[ID6854DS0$F699=="1980"]<-NA #not a feasible date
ID6854DS0$AGECALC<-ID6854DS0$F699-ID6854DS0$F970A

#Income, Education, Occupation ----
##Income
ID6854DS0$F4643[ID6854DS0$F4643=="99999998"]<-NA
ID6854DS0$F4643[ID6854DS0$F4643=="99999999"]<-NA
ID6854DS0$F4676[ID6854DS0$F4676=="99999998"]<-NA
ID6854DS0$F4676[ID6854DS0$F4676=="99999999"]<-NA
ID6854DS0$INC<-rowSums(x=cbind(ID6854DS0$F4643,ID6854DS0$F4676),na.rm = TRUE)
        
##Education
ID6854DS0$F987A[ID6854DS0$F987A=="97"]<-NA
ID6854DS0$F987A[ID6854DS0$F987A=="98"]<-NA
ID6854DS0$F987A[ID6854DS0$F987A=="99"]<-NA
ID6854DS0$EDU<-ID6854DS0$F987A
...
write.csv(ID6854DS0, file = 'ID6854DS0_1998_recoded.csv')
...
ID6854DS0_1998_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID6854DS0_1998_aggregated, file = 'ID6854DS0_1998_aggregated.csv',row.names = FALSE)
