#load dataset
library('foreign')
da04176.0004<-read.spss('04176-0004-Data.sav')
#merge with income data
da04176.0002<-read.spss('04176-0002-Data.sav')
merged4176<-merge(da04176.0004,da04176.0002,by="HHX")
#Coded variables
#Participant ID: HHX
...
ID4176DS4<-data.frame(HHX,SAD,NERVOUS,RESTLESS,HOPELESS,EFFORT,WORTHLS,ORIGIN_I,RCDT1P_I,
                      SEX,AGE_P,SRVY_YR.x,FINCGRP,EDUC,OCCUP1) 
detach(merged4176)
colnames(ID4176DS4)[colnames(ID4176DS4)=="SRVY_YR.x"]<-"SRVY_YR"
ID4176DS4$ORIGOBS<-nrow(ID4176DS4)
rm(da04176.0004,da04176.0002)
...
ID4176DS4$SAD<-ifelse(ID4176DS4$SAD=="ALL of the time",4,
                  ifelse(ID4176DS4$SAD=="MOST of the time",3,
                  ifelse(ID4176DS4$SAD=="SOME of the time",2,
                  ifelse(ID4176DS4$SAD=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$SAD=="NONE of the time",0,NA)))))
ID4176DS4$NERVOUS<-ifelse(ID4176DS4$NERVOUS=="ALL of the time",4,
                  ifelse(ID4176DS4$NERVOUS=="MOST of the time",3,
                  ifelse(ID4176DS4$NERVOUS=="SOME of the time",2,
                  ifelse(ID4176DS4$NERVOUS=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$NERVOUS=="NONE of the time",0,NA)))))
ID4176DS4$RESTLESS<-ifelse(ID4176DS4$RESTLESS=="ALL of the time",4,
                  ifelse(ID4176DS4$RESTLESS=="MOST of the time",3,
                  ifelse(ID4176DS4$RESTLESS=="SOME of the time",2,
                  ifelse(ID4176DS4$RESTLESS=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$RESTLESS=="NONE of the time",0,NA)))))
ID4176DS4$HOPELESS<-ifelse(ID4176DS4$HOPELESS=="ALL of the time",4,
                  ifelse(ID4176DS4$HOPELESS=="MOST of the time",3,
                  ifelse(ID4176DS4$HOPELESS=="SOME of the time",2,
                  ifelse(ID4176DS4$HOPELESS=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$HOPELESS=="NONE of the time",0,NA)))))
ID4176DS4$EFFORT<-ifelse(ID4176DS4$EFFORT=="ALL of the time",4,
                  ifelse(ID4176DS4$EFFORT=="MOST of the time",3,
                  ifelse(ID4176DS4$EFFORT=="SOME of the time",2,
                  ifelse(ID4176DS4$EFFORT=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$EFFORT=="NONE of the time",0,NA)))))
ID4176DS4$WORTHLS<-ifelse(ID4176DS4$WORTHLS=="ALL of the time",4,
                  ifelse(ID4176DS4$WORTHLS=="MOST of the time",3,
                  ifelse(ID4176DS4$WORTHLS=="SOME of the time",2,
                  ifelse(ID4176DS4$WORTHLS=="A LITTLE of the time",1,
                  ifelse(ID4176DS4$WORTHLS=="NONE of the time",0,NA)))))
...
dsitems <- ID4176DS4[c(2:7)]
#dsitemspw <- ID4176DS4[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID4176DS4$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID4176DS4$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID4176DS4$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID4176DS4$LAT<-ifelse(ID4176DS4$ORIGIN_I == "Yes", 1, 0)
ID4176DS4$MR<-ifelse(ID4176DS4$RCDT1P_I=="Multiple detailed race*",1,0)
ID4176DS4$WHT<-ifelse(ID4176DS4$RCDT1P_I=="White only",1,0)
ID4176DS4$BLK<-ifelse(ID4176DS4$RCDT1P_I=="Black/African American only",1,0)
ID4176DS4$AI<-ifelse(ID4176DS4$RCDT1P_I=="AIAN only",1,0)
ID4176DS4$AA<-ifelse(((ID4176DS4$RCDT1P_I=="Asian Indian only")|(ID4176DS4$RCDT1P_I=="Chinese only")|
                      (ID4176DS4$RCDT1P_I=="Filipino only")|(ID4176DS4$RCDT1P_I=="Other Asian only")),1,0)
ID4176DS4$OTH<-ifelse(ID4176DS4$RCDT1P_I=="Other race only",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino/Hispanic)
#ID4176DS4$MR<-ifelse((rowSums(x=cbind(ID4176DS4$WHT,ID4176DS4$BLK,ID4176DS4$AI,ID4176DS4$AA,ID4176DS4$OTH),
#                             na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino/Hispanic)
ID4176DS4$WHT<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 0)&(ID4176DS4$WHT == 1)), 1, 0)
ID4176DS4$BLK<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 0)&(ID4176DS4$BLK == 1)), 1, 0)
ID4176DS4$AI<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 0)&(ID4176DS4$AI == 1)), 1, 0)
ID4176DS4$AA<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 0)&(ID4176DS4$AA == 1)), 1, 0)
ID4176DS4$OTH<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 0)&(ID4176DS4$OTH == 1)), 1, 0)
ID4176DS4$MR<-ifelse(((ID4176DS4$LAT == 0)&(ID4176DS4$MR == 1)), 1, 0)

#Recode factor into numeric,
ID4176DS4$SEX<-ifelse(ID4176DS4$SEX == "Male", 1, 0)

#Recode age 
levels(ID4176DS4$AGE_P)[levels(ID4176DS4$AGE_P)=="85+ years"]<-"85"
ID4176DS4$AGECALC<-as.numeric(as.character(ID4176DS4$AGE_P))

#Income, Education, Occupation ----
##Income
#Take highest income reported
ID4176DS4$INC<-ifelse(ID4176DS4$FINCGRP=="0-$4999",2499,                   
               ifelse(ID4176DS4$FINCGRP=="$5000-$9999",7499,                 
               ifelse(ID4176DS4$FINCGRP=="$10000-$14999",12499,               
               ifelse(ID4176DS4$FINCGRP=="$15000-$19999",17499,               
               ifelse(ID4176DS4$FINCGRP=="$20000-$24999",22499,              
               ifelse(ID4176DS4$FINCGRP=="$25000-$34999",29999,               
               ifelse(ID4176DS4$FINCGRP=="$35000-$44999",39999,               
               ifelse(ID4176DS4$FINCGRP=="$45000-$54999",49999,               
               ifelse(ID4176DS4$FINCGRP=="$55000-$64999",59999,               
               ifelse(ID4176DS4$FINCGRP=="$65000-$74999",69999,              
               ifelse(ID4176DS4$FINCGRP=="$75000 and over",75100,NA))))))))))) 
               
##Education
#categories are close to continuous 
ID4176DS4$EDU<-ifelse(ID4176DS4$EDUC=="Never attended/ kindergarten only",0,       
               ifelse(ID4176DS4$EDUC=="1",1,
               ifelse(ID4176DS4$EDUC=="2",2, 
               ifelse(ID4176DS4$EDUC=="3",3,                                       
               ifelse(ID4176DS4$EDUC=="4",4,                                       
               ifelse(ID4176DS4$EDUC=="5",5,                                       
               ifelse(ID4176DS4$EDUC=="6",6,                                       
               ifelse(ID4176DS4$EDUC=="7",7,                                       
               ifelse(ID4176DS4$EDUC=="8",8,                                       
               ifelse(ID4176DS4$EDUC=="9",9,  
               ifelse(ID4176DS4$EDUC=="10",10,                                      
               ifelse(ID4176DS4$EDUC=="11",11,                                      
               ifelse(ID4176DS4$EDUC=="12th grade, no diploma",11.5,                  
               ifelse(ID4176DS4$EDUC=="HIGH SCHOOL GRADUATE",12,                    
               ifelse(ID4176DS4$EDUC=="GED or equivalent",12,                      
               ifelse(ID4176DS4$EDUC=="Some college, no degree",13,                 
               ifelse(ID4176DS4$EDUC=="AA degree: technical or vocational",14,      
               ifelse(ID4176DS4$EDUC=="AA degree: academic program",14,             
               ifelse(ID4176DS4$EDUC=="Bachelor's degree (BA, AB, BS, BBA)",16,     
               ifelse(ID4176DS4$EDUC=="Master's degree (MA, MS, MEng, MEd, MBA)",18,
               ifelse(ID4176DS4$EDUC=="Professional degree (MD, DDS, DVM, JD)",18,  
               ifelse(ID4176DS4$EDUC=="Doctoral degree (PhD, EdD)",22,NA))))))))))))))))))))))
...
write.csv(ID4176DS4, file = 'ID4176DS4_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symtpoms items
ID4176DS4 <- ID4176DS4[which(ID4176DS4$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID4176DS4$AGECALC)
dstatsdep<-describe(ID4176DS4$DEP)
dstatsachv<-NA#describe(ID4176DS4$ACHV)
dstatsinc<-describe(ID4176DS4$INC)
dstatsedu<-describe(ID4176DS4$EDU)
dstatsoccs<-NA#describe(ID4176DS4$OCCS)
dstatsoccp<-NA#describe(ID4176DS4$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID4176DS4_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID4176DS4_dephist.pdf')
hist(ID4176DS4$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID4176DS4_achvhist.pdf')
#hist(ID4176DS4$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID4176DS4_inchist.pdf')
hist(ID4176DS4$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID4176DS4_educhist.pdf')
hist(ID4176DS4$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID4176DS4_occshist.pdf')
#hist(ID4176DS4$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID4176DS4_occphist.pdf')
#hist(ID4176DS4$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
#CH<-ID4176DS4[which((ID4176DS4$AGECALC >= 5)&(ID4176DS4$AGECALC <= 11)), ] 
#AD<-ID4176DS4[which((ID4176DS4$AGECALC >= 12)&(ID4176DS4$AGECALC <= 17)), ]
YD<-ID4176DS4[which((ID4176DS4$AGECALC >= 18)&(ID4176DS4$AGECALC <= 25)), ]
AT<-ID4176DS4[which((ID4176DS4$AGECALC >= 26)&(ID4176DS4$AGECALC <= 64)), ]
OA<-ID4176DS4[which(ID4176DS4$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID4176DS4, then DATAID = 21600)
#DATAID<-'4176'
#names(DATAID)<-'DATAID'
...
ID4176DS4_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID4176DS4_aggregated, file = 'ID4176DS4_aggregated.csv',row.names = FALSE)
