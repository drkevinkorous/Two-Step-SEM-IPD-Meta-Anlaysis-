...
#V5001,V5002,V5003,V5004(RC),V5005,V5006,V5007(RC),V5008,V5010,V5011,V5012
...
#Birth Month: NA
#Birth Day: NA
#Evaluation Year: V4005
#Evaluation Month: V4003
#Evaluation Day: V4004
#Participant Income: NA  
#Maternal Income: NA
#Paternal Income: NA
#Household Income: V6072
...
#Participant Occupation: V7018, V1112 (prior wave)
...
#Notes: Americans' Changing Lives - Wave 2

#Create Dataset ----
attach(da04690.0001)
#Extract relevant variables into a dataframe.
ID4690DS1<-data.frame(V5001,V5002,V5003,V5004,V5005,V5006,V5007,V5008,V5010,V5011,V5012,
                      V1002,V1003,V1005,V1006,V1007,V1008,V1010,V1011,V1012,V1013,V1014,V1620,V2004,
                      V1628,V1629,V1631,V1630,V1632,V1801,V2001,V4005,V4003,V4004,V6072,V2007,V7018,V1112) 
detach(da04690.0001)
...
#Depressive Symptoms ---------
ID4690DS1$V5001<-ifelse(ID4690DS1$V5001=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5001=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5001=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5002<-ifelse(ID4690DS1$V5002=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5002=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5002=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5003<-ifelse(ID4690DS1$V5003=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5003=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5003=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5004<-ifelse(ID4690DS1$V5004=="(1) HARDEVER",2,
                 ifelse(ID4690DS1$V5004=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5004=="(3) MOSTTIME",0,NA))) #RC
ID4690DS1$V5005<-ifelse(ID4690DS1$V5005=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5005=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5005=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5006<-ifelse(ID4690DS1$V5006=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5006=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5006=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5007<-ifelse(ID4690DS1$V5007=="(1) HARDEVER",2,
                 ifelse(ID4690DS1$V5007=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5007=="(3) MOSTTIME",0,NA))) #RC
ID4690DS1$V5008<-ifelse(ID4690DS1$V5008=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5008=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5008=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5010<-ifelse(ID4690DS1$V5010=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5010=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5010=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5011<-ifelse(ID4690DS1$V5011=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5011=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5011=="(3) MOSTTIME",2,NA)))
ID4690DS1$V5012<-ifelse(ID4690DS1$V5012=="(1) HARDEVER",0,
                 ifelse(ID4690DS1$V5012=="(2) SOMETIME",1,
                 ifelse(ID4690DS1$V5012=="(3) MOSTTIME",2,NA)))
...
#Recode age 
ID4690DS1$AGECALC<-1989-ID4690DS1$V2001

#Income, Education, Occupation ----
##Income
ID4690DS1$INC<-ID4690DS1$V6072 

##Education
ID4690DS1$EDU<-ID4690DS1$V2007

##Occupation
#merge with prior wave (respondents only answered if job changed)
ID4690DS1$occupation<-ifelse((is.na(ID4690DS1$V7018)),ID4690DS1$V1112,ID4690DS1$V7018)
#Nam-Powers-Boyd scale (Nam & Boyd, 2008)
occ_codes<-read.csv(file = 'ID4690DS1_1989_Occupational codes.csv')
#match codes with each row in data
ID4690DS1$OCCS<-occ_codes$OCC_STATUS[ match(ID4690DS1$occupation,occ_codes$OCC_CODE)]

#Recode factor into numeric (occupational prestige)
#Nakao & Tres scale (1994)
ID4690DS1$OCCP<-occ_codes$OCC_PREST[ match(ID4690DS1$occupation,occ_codes$OCC_CODE)]

#Save Recoded Dataset ------
#Save database as .csv for OSF 
write.csv(ID4690DS1, file = 'ID4690DS1_1989_recoded.csv')
...
#Compute reliability estimates
#Only need to change depressive symptoms items 
ALPHAOV <- NA#ci.reliability(data=AIATM[1:11], type = "alpha", interval.type = "none")
OMEGAOV <- NA#ci.reliability(data=AIATM[1:11], type = "omega", interval.type = "none")
ALPHA <- NA#ALPHAOV$est
OMEGA <- NA#OMEGAOV$est
#Combine columns of aggregated data and computed codes for meta-analysis (do not include individual scores here)
...
ID4690DS1_1989_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
#WHTADFAGGRTED,
#WHTADMAGGRTED,
...
write.csv(ID4690DS1_1989_aggregated, file = 'ID4690DS1_1989_aggregated.csv',row.names = FALSE)
