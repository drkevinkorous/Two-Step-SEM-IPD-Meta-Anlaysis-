#Load data
da04582.0001<-as.data.frame(foreign::read.spss('04582-0001-Data.sav'))
...
merged4582<-merge(da04582.0001,da04582.0008,by="hhid",all.x = TRUE)
...
ID4582DS1<-data.frame(respid,ndepresa,ndepresb,ndepresc,ndepresd,ndeprese,umkaeth,umkarace,
                      umkagend,umkaage,ublinc,umedulev)
detach(merged4582)
ID4582DS1$ORIGOBS<-nrow(ID4582DS1)
rm(da04582.0001,da04582.0008,merged4582)

#Check to make sure there is no funny business
head(ID4582DS1)

#Depressive Symptoms ---------
ID4582DS1$ndepresa<-dplyr::recode(ID4582DS1$ndepresa,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4582DS1$ndepresb<-dplyr::recode(ID4582DS1$ndepresb,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4582DS1$ndepresc<-dplyr::recode(ID4582DS1$ndepresc,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
ID4582DS1$ndepresd<-dplyr::recode(ID4582DS1$ndepresd,
                                 "None of the time"=3,
                                 "Some of the time"=2,          
                                 "Most of the time"=1,               
                                 "All of the time"=0)#RC
ID4582DS1$ndeprese<-dplyr::recode(ID4582DS1$ndeprese,
                                 "None of the time"=0,
                                 "Some of the time"=1,          
                                 "Most of the time"=2,               
                                 "All of the time"=3)
...
dsitems <- ID4582DS1[c(2:6)]
#dsitemspw <- ID4582DS1[c(:)]
#Identify participants missing more than half of dsitems (not dsitemsW1) (1=missing > 50%) - to be used later
ID4582DS1$dsmiss<-ifelse(rowSums(is.na(dsitems))>((ncol(dsitems)/2)),1,0)
#Compute mean symptom score 
ID4582DS1$DEP<-rowMeans(x=dsitems,na.rm=TRUE)
ID4582DS1$DEPPW<-NA#rowMeans(x=dsitemspw,na.rm=TRUE)
...
ID4582DS1$LAT<-ifelse(ID4582DS1$umkaeth=="Hispanic",1,0)
ID4582DS1$WHT<-ifelse(ID4582DS1$umkarace=="White",1,0)
ID4582DS1$BLK<-ifelse(ID4582DS1$umkarace=="Black",1,0)
ID4582DS1$AI<-NA
ID4582DS1$AA<-NA
ID4582DS1$OTH<-ifelse(ID4582DS1$umkarace=="Other",1,0)

#Compute variable for participants who reported more than one race (mixed/biracial; do not include Latino)
ID4582DS1$MR<-NA#ifelse((rowSums(x=cbind(ID4582DS1$WHT,ID4582DS1$BLK,ID4582DS1$AI,ID4582DS1$AA,
#                                       ID4582DS1$OTH),na.rm=TRUE)) > 1, 1, 0)

##Compute variables for participants who reported a single race (excluding mixed/biracial & Latino)
ID4582DS1$WHT<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$WHT == 1)), 1, 0)
ID4582DS1$BLK<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$BLK == 1)), 1, 0)
#ID4582DS1$AI<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$AI == 1)), 1, 0)
#ID4582DS1$AA<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$AA == 1)), 1, 0)
ID4582DS1$OTH<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$OTH == 1)), 1, 0)
#ID4582DS1$MR<-ifelse(((ID4582DS1$LAT == 0)&(ID4582DS1$MR == 1)), 1, 0)

#Recode sex
ID4582DS1$SEX<-ifelse(ID4582DS1$umkagend == "Male", 1, 0)

#Recode age 
ID4582DS1$AGECALC<-ID4582DS1$umkaage

#Income, Education, Occupation ----
##Income
ID4582DS1$INC<-ID4582DS1$ublinc

##Education 
ID4582DS1$EDU<-dplyr::recode(ID4582DS1$umedulev,
...
write.csv(ID4582DS1, file = 'ID4582DS1_recoded.csv')

#Descriptives and Correlations -----
#Exclude participants with more than 50% missing on depressive symptoms items
ID4582DS1 <- ID4582DS1[which(ID4582DS1$dsmiss==0), ]

##Compute mean and sd across all participants within dataset
library('psych')
#Means & SDs of focal variables
dstatsage<-describe(ID4582DS1$AGECALC)
dstatsdep<-describe(ID4582DS1$DEP)
dstatsachv<-NA#describe(ID4582DS1$ACHV)
dstatsinc<-describe(ID4582DS1$INC)
dstatsedu<-describe(ID4582DS1$EDU)
dstatsoccs<-NA#describe(ID4582DS1$OCCS)
dstatsoccp<-NA#describe(ID4582DS1$OCCP)

#Save descriptive stats
dstats<-rbind(dstatsage,dstatsdep,dstatsachv,dstatsinc,dstatsedu,dstatsoccs,dstatsoccp,make.row.names= FALSE)
write.csv(dstats,file = 'ID4582DS1_dstats.csv',
          row.names=c("dstatsage","dstatsdep","dstatsachv","dstatsinc","dstatsedu","dstatsoccs","dstatsoccp"))

##Histograms 
pdf('ID4582DS1_dephist.pdf')
hist(ID4582DS1$DEP, xlab="Depressive Symptoms", main = NULL, col="grey")
dev.off()
#pdf('ID4582DS1_achvhist.pdf')
#hist(ID4582DS1$ACHV, xlab="Academic Achievement", main = NULL, col="grey")
#dev.off()
pdf('ID4582DS1_inchist.pdf')
hist(ID4582DS1$INC, xlab="Income", main = NULL, col="grey")
dev.off()
pdf('ID4582DS1_educhist.pdf')
hist(ID4582DS1$EDU, xlab="Educational Attainment", main = NULL, col="grey")
dev.off()
#pdf('ID4582DS1_occshist.pdf')
#hist(ID4582DS1$OCCS, xlab="Occupational Status", main = NULL, col="grey")
#dev.off()
#pdf('ID4582DS1_occphist.pdf')
#hist(ID4582DS1$OCCP, xlab="Occupational Prestige", main = NULL, col="grey")
#dev.off()
...
table(ID4582DS1$AGECALC)
#Separate dataset by developmental period, if needed
#CH<-ID4582DS1[which((ID4582DS1$AGECALC >= 5)&(ID4582DS1$AGECALC <= 11)), ] 
AD<-ID4582DS1[which((ID4582DS1$AGECALC >= 12)&(ID4582DS1$AGECALC <= 17)), ]
YD<-ID4582DS1[which((ID4582DS1$AGECALC >= 18)&(ID4582DS1$AGECALC <= 25)), ]
AT<-ID4582DS1[which((ID4582DS1$AGECALC >= 26)&(ID4582DS1$AGECALC <= 64)), ]
OA<-ID4582DS1[which(ID4582DS1$AGECALC >= 65), ]
...
##Assign ICPSR ID number (ICPSR Number; e.g., file name = ID4582DS1, then DATAID = 21600)
#DATAID<-'4582'
#names(DATAID)<-'DATAID'
...
ID4582DS1_aggregated<-dplyr::bind_rows(
#WHTCHFAGGRTED,
#WHTCHMAGGRTED,
WHTADFAGGRTED,
WHTADMAGGRTED,
...
write.csv(ID4582DS1_aggregated, file = 'ID4582DS1_aggregated.csv',row.names = FALSE)